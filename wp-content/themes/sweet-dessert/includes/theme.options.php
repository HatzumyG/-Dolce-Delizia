...
if ( !function_exists( 'sweet_dessert_options_settings_theme_setup' ) ) {
	...
	function sweet_dessert_options_settings_theme_setup() {
        ...
        sweet_dessert_storage_set('options',     apply_filters('sweet_dessert_filter_options', array(
		...
		"blog_counters" => array(
					...
					"desc" => wp_kses_data( __('Select counters, displayed near the post title. Please note that this option will only work with the active TRX Utils plugin.', 'sweet-dessert') ),
					"override" => "category,services_group,page,custom",
					"std" => "comments",
		...
		"show_reviews" => array(
					...
					"desc" => wp_kses_data( __("Show reviews block on single post page and average reviews rating after post's title in stream pages. Please note that this option will only work with the active TRX Utils plugin.", 'sweet-dessert') ),
					"override" => "category,services_group,services_group",
					"std" => "no",
					"options" => sweet_dessert_get_options_param('list_yes_no'),
					"type" => "switch"),
		...
		"social_icons" => array(
					...
					"desc" => wp_kses_data(  esc_html__("Add button's code for each social share network.",'sweet-dessert')).'<br>'
							.wp_kses_data(  esc_html__("In share url you can use next macro:",'sweet-dessert')).'<br>'
							.wp_kses_data(  sprintf(esc_html__("%s - share post (page) URL,",'sweet-dessert'), '<b>{url}</b>')).'<br>'
							.wp_kses_data(  sprintf(esc_html__("%s - post title,",'sweet-dessert'), '<b>{title}</b>')).'<br>'
							.wp_kses_data(  sprintf(esc_html__("%s - post image,",'sweet-dessert'), '<b>{image}</b>')).'<br>'
							.wp_kses_data(  sprintf(esc_html__("%s - post description (if supported)",'sweet-dessert'), '<b>{descr}</b>')).'<br>'
							.wp_kses_data(  esc_html__("For example:",'sweet-dessert')).'<br>'
							.wp_kses_data(  sprintf(esc_html__("%s share string: %s",'sweet-dessert'), '<b>Facebook</b>','<em>http://www.facebook.com/sharer.php?u={link}&amp;t={title}</em>' )).'<br>'
							.wp_kses_data(  sprintf(esc_html__("%s share string: %s",'sweet-dessert'), '<b>Delicious</b>','<em>http://delicious.com/save?url={link}&amp;title={title}&amp;note={descr}</em>' )),
					"override" => "category,services_group,page,custom",
					"type" => "info"),
		...
		"share_buttons" => array(
					"title" => esc_html__('Share buttons',  'sweet-dessert'),
					"desc" => wp_kses_data( __('Select icon and write share URL for desired social networks.', 'sweet-dessert')).'<br>'
					.wp_kses_data(  sprintf(esc_html__("%sImportant!%s If you leave text field empty - internal theme link will be used (if present).", 'sweet-dessert'), '<b>','</b>')),					
					"dependency" => array(
						'show_share' => array('vertical', 'horizontal')
					),
					"std" => array(array('url'=>'', 'icon'=>'')),
		...
		"info_socials_3" => array(
					...
					"desc" => wp_kses_data( __("Put to this section Twitter API 1.1 keys.", 'sweet-dessert')).'<br>'
					.wp_kses_data(  sprintf(esc_html__("You can take them after registration your application in %s", 'sweet-dessert'), '<strong>https://apps.twitter.com/</strong>')),
					"type" => "info"),
		...
		'api_google' => array(
					...
					"desc" => wp_kses_data( __("Insert Google API Key for browsers into the field above to generate Google Maps. Please note that this option will only work with the active TRX Utils plugin.", 'sweet-dessert') ),
					"std" => "",
					"type" => "text"),
		...
		"admin_add_filters" => array(
					...
					"desc" => wp_kses_data( __('Show additional filters (on post formats, tags and categories) in admin panel page "Posts".', 'sweet-dessert')). '<br>'
					.wp_kses_data( __('Attention! If you have more than 2.000-3.000 posts, enabling this option may cause slow load of the "Posts" page! If you encounter such slow down, simply open Appearance - Theme Options - Service and set "No" for this option.', 'sweet-dessert') ),
					"std" => "no",
					"options" => sweet_dessert_get_options_param('list_yes_no'),
					"type" => "switch"),
        ...
        "disable_widgets_block_editor" => array(
                "title" => esc_html__('Disable new Widgets Block Editor', 'sweet-dessert'),
                "desc" => wp_kses_data( __('Attention! If after the update to WordPress 5.8+ you are having trouble editing widgets or working in Customizer - disable new Widgets Block Editor (used in WordPress 5.8+ instead of a classic widgets panel)', 'sweet-dessert') ),
                "std" => "no",
                "options" => sweet_dessert_get_options_param('list_yes_no'),
                "type" => "switch"),
...
if ( !function_exists( 'sweet_dessert_options_reset' ) ) {
	function sweet_dessert_options_reset($clear=true) {
		 ...
		 if (is_dir(SWEET_DESSERT_THEME_PATH . 'demo/')) {
			  $demo_url = SWEET_DESSERT_THEME_PATH . 'demo/';
		 } else {
			  $demo_url = esc_url(sweet_dessert_get_protocol() . '://demofiles.axiomthemes.com/sweet-dessert/'); // Demo-site domain
		 }
		 ...
		 if (get_option($option_name, false) === false) {
			  if ($clear) {
                    ...
                    $txt = sweet_dessert_fgc($demo_url . 'default/templates_options.txt');
					if ($txt) {

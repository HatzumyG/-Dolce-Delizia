...
if ( ! function_exists( 'sweet_dessert_fix_urls' ) ) {
    add_action( 'init', 'sweet_dessert_fix_urls' );
    add_action( 'trx_utils_action_importer_import_end', 'sweet_dessert_fix_urls', 10, 1 );
    /**
     * Replace the URLs with a theme demo URL to the current site URL.
     *
     * @hooked init
     * @hooked trx_utils_action_importer_import_end
     */
    function sweet_dessert_fix_urls( $importer = false ) {
        if ( ! is_admin() || ( (int)get_option( 'sweet_dessert_fix_urls' ) > 0 && current_action() != 'trx_utils_action_importer_import_end' ) ) {
            return;
        }
        sweet_dessert_fix_urls_in_attachments();
        sweet_dessert_fix_urls_in_elementor();
        sweet_dessert_fix_urls_in_vc();
        sweet_dessert_fix_urls_in_revslider();
        update_option( 'sweet_dessert_fix_urls', 1 );
    }
}

if ( ! function_exists( 'sweet_dessert_fix_urls_in_attachments' ) ) {
    /**
     * Replace the URLs with a theme demo URL to the current site URL in the Elementor data.
     */
    function sweet_dessert_fix_urls_in_attachments() {
        global $wpdb;
...
if ( ! function_exists( 'sweet_dessert_fix_urls_in_elementor' ) ) {
    /**
     * Replace the URLs with a theme demo URL to the current site URL in the Elementor data.
     */
    function sweet_dessert_fix_urls_in_elementor() {
        global $wpdb;
...
if ( ! function_exists( 'sweet_dessert_fix_urls_in_revslider' ) ) {
    /**
     * Replace the URLs with a theme demo URL to the current site URL in the revslider tables
     */
    function sweet_dessert_fix_urls_in_revslider() {
        global $wpdb;
                    ...
                    foreach ( $rows as $k => $row ) {
                        foreach ( $fields as $field ) {
                            if ( ! empty( $row[ $field ] ) ) {
                                ...
                                if ( is_array( $row[ $field ] ) && sweet_dessert_fix_url( $row[ $field ] ) ) {
...
if ( ! function_exists( 'sweet_dessert_fix_url' ) ) {
    /**
     ...
     */
    function sweet_dessert_fix_url( &$data ) {
        ...
        if ( $home_url === false ) {
            ...
            $home_url = home_url();
            // Demo-site domain
            $demo_url = sweet_dessert_storage_get( 'theme_demo_url' );
            if ( substr( $demo_url, 0, 2 ) === '//' ) {
                $demo_url = sweet_dessert_fix_get_protocol() . ':' . $demo_url;
            }
            if ( empty( $demo_url ) ) {
                $importer_options = apply_filters( 'trx_utils_filter_importer_options', array(
        ...
        if ( ! empty( $demo_url ) ) {
            if ( is_array( $data ) ) {
                foreach ( $data as $k => $v ) {
                    if ( is_array( $v ) ) {
                        $rez = sweet_dessert_fix_url( $data[ $k ] );
                        $changed = $changed || $rez;
                    } else if ( ! empty( $v ) && is_string( $v ) ) {
                        $data[ $k ] = sweet_dessert_fix_url_replace( $demo_url, $home_url, $v );
                        $changed = $changed || $data[ $k ] != $v;
                    }
                }
            } else if ( ! empty( $data ) && is_string( $data ) ) {
                $data_new = sweet_dessert_fix_url_replace( $demo_url, $home_url, $data );
...
if ( ! function_exists('sweet_dessert_fix_url_replace') ) {
    /**
     ...
     */
    function sweet_dessert_fix_url_replace($from, $to, $str) {
        if ( substr($from, -1) == '/' ) {
        ...
        $from_clear = sweet_dessert_fix_remove_protocol_from_url($from, true);
        $to_clear = sweet_dessert_fix_remove_protocol_from_url($to, true);
        return sweet_dessert_fix_str_replace(
            array(
            ...
            array(
                /* 1 */					urlencode(sweet_dessert_fix_get_protocol() . "://{$to_clear}"),
                /* 2 */					urlencode(sweet_dessert_fix_get_protocol() . "://{$to_clear}"),
                /* 3 */					urlencode($to),
                /* 4 */					urlencode("//{$to_clear}"),
                /* 5 */					sweet_dessert_fix_get_protocol() . "://{$to_clear}",
                /* 6 */					str_replace('/', '\\/', sweet_dessert_fix_get_protocol() . "://{$to_clear}"),
                /* 7 */					sweet_dessert_fix_get_protocol() . "://{$to_clear}",
                /* 8 */					str_replace('/', '\\/', sweet_dessert_fix_get_protocol() . "://{$to_clear}"),
                /* 9 */					$to,
                /* 10 */				str_replace('/', '\\/', $to),
                /* 11 */				"//{$to_clear}",
...
if ( ! function_exists( 'sweet_dessert_fix_remove_protocol_from_url' ) ) {
    /**
     ...
     */
    function sweet_dessert_fix_remove_protocol_from_url( $url, $complete = true ) {
        return preg_replace( '/(http[s]?:)?' . ( $complete ? '\\/\\/' : '' ) . '/', '', $url );
    }
}

if ( ! function_exists( 'sweet_dessert_fix_get_protocol' ) ) {
    /**
     * Return a current protocol ( http or https ) of the site.
     *
     * @return string A string with a protocol.
     */
    function sweet_dessert_fix_get_protocol( $suffix = false ) {
        return ( is_ssl() ? 'https' : 'http' ) . ( ! empty( $suffix ) ? ':' : '' );
    }
}

if ( ! function_exists( 'sweet_dessert_fix_str_replace' ) ) {
    /**
     ...
     */
    function sweet_dessert_fix_str_replace( $from, $to, $str ) {
        if ( is_array( $str ) ) {
            foreach ( $str as $k => $v ) {
                $str[ $k ] = sweet_dessert_fix_str_replace( $from, $to, $v );
            }
            ...
            if ( is_serialized( $str ) ) {
                $str = serialize( sweet_dessert_fix_str_replace( $from, $to, sweet_dessert_fix_unserialize( $str ) ) );
            } else {
...
if ( ! function_exists( 'sweet_dessert_fix_unserialize_recover' ) ) {
    /**
     ...
     * @return string      A processed string.
     */
    function sweet_dessert_fix_unserialize_recover( $str ) {
...
if ( ! function_exists( 'sweet_dessert_fix_unserialize' ) ) {
    /**
     ...
     */
    function sweet_dessert_fix_unserialize( $str ) {
        if ( ! empty( $str ) && is_serialized( $str ) ) {
            ...
            if ( true || ! preg_match( '/O:[0-9]+:"([^"]*)":[0-9]+:{/', $str, $matches ) || empty( $matches[1] ) || class_exists( $matches[1] ) ) {
                try {
                    ...
                    if ( false === $data ) {
                        $data = @unserialize( sweet_dessert_fix_unserialize_recover( $str ) );
                    }
                    // Attempt 4: try unserialize original string without CR symbol '\r' with modified character counters
                    if ( false === $data ) {
                        $data = @unserialize( sweet_dessert_fix_unserialize_recover( $str2 ) );
                    }
                } catch ( Exception $e ) {
                    if (  sweet_dessert_get_theme_option( 'debug_mode', false  ) ) {
                        dcl( $e->getMessage() );
                    }
                    $data = false;
                }

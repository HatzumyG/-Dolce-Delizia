...
if ( !function_exists( 'sweet_dessert_options_load_scripts' ) ) {
	function sweet_dessert_options_load_scripts() {
		...
		if (!$loaded) {
			...
			wp_enqueue_script('jquery-colors-script',		sweet_dessert_get_file_url('js/colorpicker/colors.js'), array('jquery'), null, true );
			wp_enqueue_script('jquery-colorpicker-script',	sweet_dessert_get_file_url('js/colorpicker/jqColorPicker.js'), array('jquery'), null, true );
				...
				if ( !in_array($field['type'], array('divider'))) {
					echo '<label class="sweet_dessert_options_field_label'
						...
						. (!empty($field['info']) && ($fdir=sweet_dessert_get_file_url('images/to_info/'.sanitize_file_name($id).'.jpg'))!=''
							? '<a href="'.esc_url($fdir).'" data-rel="popup" target="_blank" class="sweet_dessert_options_field_label_info iconadmin-info-circled" title="'. esc_attr__('More info', 'sweet-dessert').'"></a>' 
							: '')
...
if ( !function_exists( 'sweet_dessert_options_save' ) ) {
	function sweet_dessert_options_save() {
		...
		$override = empty($_POST['override']) ? 'general' : sweet_dessert_get_value_gpc('override');
		$slug = empty($_POST['slug']) ? '' : sweet_dessert_get_value_gpc('slug');
		...
		if ($mode == 'save') {
			parse_str(sweet_dessert_get_value_gpc('data'), $post_data);
		} else if ($mode=='export') {
			parse_str(sweet_dessert_get_value_gpc('data'), $post_data);
		...
		if ($mode=='export') {
			$name  = sanitize_text_field(trim(chop($_POST['name'])));
...
if ( !function_exists( 'sweet_dessert_options_import' ) ) {
	function sweet_dessert_options_import() {
		...
		$override = $_POST['override']=='' ? 'general' : sweet_dessert_get_value_gpc('override');
			...
			foreach ($post_options as $id=>$field) { 
				...
				if ($mode == 'save' || $mode=='export') {
					if ($override!='general' && sweet_dessert_is_inherit_option($post_data[$id.'_inherit']))
						...
						if ($id == 'reviews_marks' && is_array($new) && function_exists('sweet_dessert_reviews_theme_setup')) {
							$new = join(',', $new);

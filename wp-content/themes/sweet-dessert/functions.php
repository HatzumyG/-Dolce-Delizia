...
if ( !function_exists( 'sweet_dessert_theme_setup' ) ) {
	...
	function sweet_dessert_theme_setup() {
		...
		add_filter( 'comment_form_fields', 'sweet_dessert_comment_form_agree', 11 );
...
if ( !function_exists( 'sweet_dessert_add_required_plugins' ) ) {
	function sweet_dessert_add_required_plugins($plugins) {
		$plugins[] = array(
			...
			'version'	=> '3.2.9',					// Minimal required version
			'slug' 		=> 'trx_utils',
...
if ( !function_exists('sweet_dessert_head_add_page_preloader_styles') ) {
    function sweet_dessert_head_add_page_preloader_styles($css) {
        if (($preloader=sweet_dessert_get_theme_option('page_preloader'))!='') {
            ...
            wp_add_inline_style('sweet-dessert-main-style', $custom_style);
        }
    }
}
...
if (!function_exists('sweet_dessert_body_add_gtm')) {
	function sweet_dessert_body_add_gtm() {
		if (sweet_dessert_get_custom_option('gtm_code')) {
...
if (!function_exists('sweet_dessert_footer_add_custom_html')) {
	function sweet_dessert_footer_add_custom_html() {
		?><div class="custom_html_section"><?php
			if (sweet_dessert_get_custom_option('custom_code')) {
...
if (!function_exists('sweet_dessert_footer_add_gtm2')) {
	function sweet_dessert_footer_add_gtm2() {
		if(sweet_dessert_get_custom_option('gtm_code2')) {
...
if(!function_exists('sweet_dessert_add_body_class')) {
     if(!function_exists ( 'trx_utils_require_shortcode')){
        add_filter( 'body_class', 'sweet_dessert_add_body_class' );
        function sweet_dessert_add_body_class($classes){
...
if ( ! function_exists( 'sweet_dessert_single_comments_field' ) ) {
	function sweet_dessert_single_comments_field( $args ) {
		 ...
		 if ( 'text' == $args['field_type'] ) {
			  ...
			  $html .= '<input id="' . esc_attr( $args['field_name'] ) . '" name="' . esc_attr( $args['field_name'] ) . '" type="checkbox" value="' . esc_attr( $args['field_value'] ) . '"' . ( $args['field_req'] ? ' aria-required="true"' : '' ) . ' />'
					. ' <label for="' . esc_attr( $args['field_name'] ) . '" class="' . esc_attr( $args['field_req'] ? 'required' : 'optional' ) . '">' . wp_kses( $args['field_title'], 'ipharm_kses_content' ) . '</label>';
		 } else {
		 ...
		 if ( 'default' != $args['form_style'] && in_array( $args['field_type'], array( 'text', 'textarea' ) ) ) {
			  $html .= '<span class="sc_form_field_hover">'
					...
					. '<span class="sc_form_field_content" data-content="' . esc_attr( $args['field_title'] ) . '">' . wp_kses( $args['field_title'], 'ipharm_kses_content' ) . '</span>'
					. '</span>';
		 }
		 $html .= '</span></div>';
		 return $html;
...
if ( ! function_exists( 'sweet_dessert_comment_form_agree' ) ) {
	function sweet_dessert_comment_form_agree( $comment_fields ) {
		 $privacy_text = sweet_dessert_get_privacy_text();
		 if ( ! empty( $privacy_text )
			&& ( ! function_exists( 'sweet_dessert_exists_gdpr_framework' ) || ! sweet_dessert_exists_gdpr_framework() )
			&& ( ! function_exists( 'sweet_dessert_exists_wp_gdpr_compliance' ) || ! sweet_dessert_exists_wp_gdpr_compliance() )	
		 ) {
			  $comment_fields['i_agree_privacy_policy'] = sweet_dessert_single_comments_field(
					array(
						 'form_style'        => 'default',
						 'field_type'        => 'checkbox',
						 'field_req'         => '',

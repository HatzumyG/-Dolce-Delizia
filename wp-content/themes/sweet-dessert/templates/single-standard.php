...
if ( !function_exists( 'sweet_dessert_template_single_standard_output' ) ) {
	function sweet_dessert_template_single_standard_output($post_options, $post_data) {
		...
		if ($show_title && $post_options['location'] == 'center' && sweet_dessert_get_custom_option('show_page_title')=='yes') {
			?>
			<<?php echo esc_attr($title_tag); ?> itemprop="<?php echo (float) $avg_author > 0 || (float) $avg_users > 0 ? 'itemReviewed' : 'headline'; ?>" class="post_title entry-title"><span class="post_icon <?php echo esc_attr($post_data['post_icon']); ?>"></span><?php sweet_dessert_show_layout($post_data['post_title']); ?></<?php echo esc_attr($title_tag); ?>>
		<?php 
		}
		...
		if ($post_data['post_protected']) { 
			...
			if (function_exists('sweet_dessert_sc_reviews') && !sweet_dessert_storage_empty('reviews_markup') && sweet_dessert_strpos($post_data['post_content'], sweet_dessert_get_reviews_placeholder())===false)
            ...
            if (function_exists('sweet_dessert_reviews_wrapper')) {
                ...
                sweet_dessert_show_layout($post_data['post_content']);
            }
                wp_link_pages( array(

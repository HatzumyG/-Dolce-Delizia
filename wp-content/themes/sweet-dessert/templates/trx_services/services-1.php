...
if ( !function_exists( 'sweet_dessert_template_services_1_theme_setup' ) ) {
	...
	function sweet_dessert_template_services_1_theme_setup() {
		sweet_dessert_add_template(array(
			...
			'template' => 'services-1',
			'mode'   => 'services',
			'title'  => esc_html__('Services /Style 1/', 'sweet-dessert'),
...
// Template output
if ( !function_exists( 'sweet_dessert_template_services_1_output' ) ) {
	function sweet_dessert_template_services_1_output($post_options, $post_data) {
		...
		<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
			...
			<?php 
			// NUEVO BLOQUE DE ÍCONO PERSONALIZADO
			$custom_icon = get_post_meta($post_data['post_id'], 'post_icon', true);
			$icon_file = $custom_icon ? $custom_icon : 'default';
			$icon_url = 'https://dolcedelizia.shop/wp-content/uploads/2025/05/' . $icon_file . '.png';

			if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
				echo '<a href="' . esc_url($post_data['post_link']) . '"><img src="' . esc_url($icon_url) . '" alt="' . esc_attr($post_data['post_title']) . '" style="width:130px; height:auto; display:block; margin:0 auto;"></a>';
			} else {
				echo '<img src="' . esc_url($icon_url) . '" alt="' . esc_attr($post_data['post_title']) . '" style="width:140px; height:auto; display:block; margin:0 auto;">';
			}
			?>

...
if ( !function_exists( 'sweet_dessert_template_attachment_theme_setup' ) ) {
	add_action( 'sweet_dessert_action_before_init_theme', 'sweet_dessert_template_attachment_theme_setup', 1 );
	function sweet_dessert_template_attachment_theme_setup() {
		sweet_dessert_add_template(array(
			'layout' => 'attachment',
			'mode'   => 'internal',
			'title'  => esc_html__('Attachment page layout', 'sweet-dessert'),
...
// Template output
if ( !function_exists( 'sweet_dessert_template_attachment_output' ) ) {
	function sweet_dessert_template_attachment_output($post_options, $post_data) {
		...
		<article <?php post_class('post_item post_item_attachment template_attachment'); ?>>
			...
			<div class="post_featured">
				<div class="post_thumb post_nav" data-image="<?php echo esc_url($post_data['post_attachment']); ?>" data-title="<?php echo esc_attr($post_data['post_title']); ?>">
					...
					if ( isset( $attachments[ $k-1 ] ) ) {
						...
						$desc = sweet_dessert_strshort(!empty($attachments[ $k-1 ]->post_excerpt) ? $attachments[ $k-1 ]->post_excerpt : $attachments[ $k-1 ]->post_title, 30);
						?>
						<a class="post_nav_item post_nav_prev" href="<?php echo esc_url($link); ?>">
							<span class="post_nav_info">
								<span class="post_nav_info_title"><?php esc_html_e('Previous item', 'sweet-dessert'); ?></span>
								<span class="post_nav_info_description"><?php sweet_dessert_show_layout($desc); ?></span>
							</span>
						</a>
						<?php
					}
					if ( isset( $attachments[ $k+1 ] ) ) {
						...
						<a class="post_nav_item post_nav_next" href="<?php echo esc_url($link); ?>">
							<span class="post_nav_info">
								<span class="post_nav_info_title"><?php esc_html_e('Next item', 'sweet-dessert'); ?></span>
								<span class="post_nav_info_description"><?php sweet_dessert_show_layout($desc); ?></span>
							</span>
						</a>
						<?php
			...
			<div class="post_content">
				...
				if ($post_data['post_protected']) { 
					...
					echo !empty($post_data['post_content']) ? trim($post_data['post_content']) : esc_html__('No image description ...', 'sweet-dessert'); 
					wp_link_pages( array( 
						'before' => '<div class="nav_pages_parts"><span class="pages">' . esc_html__( 'Pages:', 'sweet-dessert' ) . '</span>', 
						'after' => '</div>',

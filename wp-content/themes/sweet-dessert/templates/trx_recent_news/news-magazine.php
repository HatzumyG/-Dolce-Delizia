...
if ( !function_exists( 'sweet_dessert_template_news_magazine_theme_setup' ) ) {
	add_action( 'sweet_dessert_action_before_init_theme', 'sweet_dessert_template_news_magazine_theme_setup', 1 );
	function sweet_dessert_template_news_magazine_theme_setup() {
		sweet_dessert_add_template(array(
			'layout' => 'news-magazine',
			'template' => 'news-magazine',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Magazine/', 'sweet-dessert'),
...
// Template output
if ( !function_exists( 'sweet_dessert_template_news_magazine_output' ) ) {
	function sweet_dessert_template_news_magazine_output($post_options, $post_data) {
				...
				<div class="post_header entry-header">
					...
					if ($show_title) {
						if (!isset($post_options['links']) || $post_options['links']) {
							?>
							<<?php echo esc_attr($title_tag); ?> class="post_title entry-title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php sweet_dessert_show_layout($post_data['post_title']); ?></a></<?php echo esc_attr($title_tag); ?>>
							<?php
						} else {
							?>
							<<?php echo esc_attr($title_tag); ?> class="post_title entry-title"><?php sweet_dessert_show_layout($post_data['post_title']); ?></<?php echo esc_attr($title_tag); ?>>
							<?php
						}
					}
					
					if ( in_array( $post_data['post_type'], array( 'post', 'attachment' ) ) ) {
						?><div class="post_meta"><span class="post_meta_author"><?php sweet_dessert_show_layout($post_data['post_author_link']); ?></span><?php
			...
			if ($number <= $featured) {
					...
					if ($post_data['post_protected']) {
						...
						if ($post_data['post_excerpt']) {
							echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
									...
									: wpautop(sweet_dessert_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																								? $post_options['descr'] 
																								: sweet_dessert_get_custom_option('post_excerpt_maxlength_masonry')
																)
											);
						}
					}
				...
				<div class="post_footer entry-footer">
					...
					if ( in_array( $post_data['post_type'], array( 'post', 'attachment' ) ) ) {
						...
						sweet_dessert_template_set_args('counters', array(
							'post_options' => $post_options,
							'post_data' => $post_data
						));
						get_template_part(sweet_dessert_get_file_slug('templates/_parts/counters.php'));
					}
					?>
					</div>
				</div><!-- .entry-footer -->

...
if ( !function_exists( 'sweet_dessert_template_single_team_output' ) ) {
	function sweet_dessert_template_single_team_output($post_options, $post_data) {
		...
		if ($show_title && $post_options['location'] == 'center' && sweet_dessert_get_custom_option('show_page_title')=='no') {
			?>
			<<?php echo esc_attr($title_tag); ?> itemprop="headline" class="post_title entry-title"><span class="post_icon <?php echo esc_attr($post_data['post_icon']); ?>"></span><?php sweet_dessert_show_layout($post_data['post_title']); ?></<?php echo esc_attr($title_tag); ?>>
			<?php 
		}
			...
			<section class="single_team_post_description">
				...
				<div class="team_meta"><?php
					if (!empty($post_meta['team_member_email'])) {
						?><p><?php esc_html_e('E-mail: ', 'sweet-dessert'); ?><?php sweet_dessert_show_layout($post_meta['team_member_email']); ?></p><?php
					}
					if (!empty($post_meta['team_member_bday'])) {
						?><p><?php esc_html_e('Date Of Birth: ', 'sweet-dessert'); ?><?php sweet_dessert_show_layout($post_meta['team_member_bday']); ?></p><?php
					}
				?></div><?php
				...
				if (!empty($post_meta['team_member_brief_info'])) {
					...
					<div class="team_brief_info">
						<h5 class="team_brief_info_title"><?php esc_html_e('Brief info', 'sweet-dessert'); ?></h5>
		...
		if ($show_title && $post_options['location'] != 'center' && sweet_dessert_get_custom_option('show_page_title')=='no') {
			?>
			<<?php echo esc_attr($title_tag); ?> itemprop="name" class="post_title entry-title"><span class="post_icon <?php echo esc_attr($post_data['post_icon']); ?>"></span><?php sweet_dessert_show_layout($post_data['post_title']); ?></<?php echo esc_attr($title_tag); ?>>
			<?php 
		}

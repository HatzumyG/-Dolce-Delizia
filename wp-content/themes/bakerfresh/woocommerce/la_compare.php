...
<div id="la_compare_table_wrapper">
    ...
    $lists = Bakerfresh_WooCommerce_Compare::get_data();

    $default_attr = Bakerfresh_WooCommerce_Compare::get_default_attributes();
    $tax_attr = Bakerfresh_WooCommerce_Compare::get_taxonomies();
    $all_attr = array_merge($default_attr, $tax_attr);

    $attribute_allow = [];
    $s = bakerfresh_get_theme_mod('compare_attribute');
    $settings = [];
    if(!empty($s)){
        $settings = explode(',', $s);
    ...
    add_filter('bakerfresh/filter/enable_image_lazyload', '__return_false');

    if (!empty($lists)) {
        foreach ($lists as $product_id) {
            $product_id = bakerfresh_wpml_object_id($product_id, 'product', true);
            $term = array();
            $_product = wc_get_product($product_id);
            $term['id'] = $product_id;
            $term['sku'] = ($_product->get_sku() ? $_product->get_sku() : esc_html__('N/A', 'bakerfresh'));
        ...
        <table class="la-compare-table la-compare-table-items <?php if($total_product > 3){ echo 'show-vtb'; } ?> ">
            ...
            foreach( $attribute_allow as $lk => $lb ){
                ...
                if(in_array($lk, array('image','title','add-to-cart'))){
                    $label_list['info'] = esc_html_x('Product Info', 'front-view', 'bakerfresh');
                }
                else{
                ...
                foreach ($terms as $item) {
                    ...
                    elseif ($k == 'stock') {
                        ...
                        if ($item['availability'] && !empty($item['availability']['availability'])) {
                            ...
                            if ($item['stock']) {
                                $text_stock = esc_html__('In stock', 'bakerfresh');
                                $class_stock = 'in-stock';
                            } else {
                                $text_stock = esc_html__('Out of stock', 'bakerfresh');
        ...
        ?>
        <p class="text-center"><?php esc_html_e('No products were added to the list', 'bakerfresh') ?></p>
        <?php
    }
    ?>
</div>

	...
	jQuery(document).ready(function($){
		...
		jQuery('#great-caroussel').sortable({
			...
			update: function( event, ui ) {
				...
				jQuery.post(ajaxurl, {action: 'gc_order_content', id: jQuery(ui.item).attr('rel'), order: (ui.item.index()+1), _ajax_nonce: '<?php echo esc_attr(wp_create_nonce( "order_content_gc" )); ?>' });
	    ...
	    jQuery('#great-caroussel img.remove').click(function(){
	    	...
	    	jQuery.post(ajaxurl, {action: 'gc_remove_content', id: jQuery(_this).attr('rel'), _ajax_nonce: '<?php echo esc_attr(wp_create_nonce( "remove_content_gc" )); ?>'}, function(){
...
<h2><?php echo esc_html($caroussel->name) ?></h2>

<form action="" method="post" id="form_new_gc">

	<?php wp_nonce_field( 'new_content_gc' ) ?>

	<input type="hidden" name="id" value="<?php echo (int)$caroussel->id ?>" />
...
<?php
...

	{
...

		{
			...
			echo '<li rel="'.(int)$content->id.'"><img class="remove" rel="'.(int)$content->id.'" src="'.esc_url(plugins_url( 'images/remove.png', dirname(__FILE__))).'" />

			<form class="form_edit_gc_content">';

				wp_nonce_field( 'update_content_gc_'.$content->id, "_wpnonce", true );

				echo '<input type="hidden" name="id" value="'.(int)$content->id.'" />
				...
				<input type="submit" value="Save" class="button-primary" /><img src="'.esc_url(plugins_url( 'images/loading.gif', dirname(__FILE__))).'" class="loading" />

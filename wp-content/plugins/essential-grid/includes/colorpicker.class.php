...
class ESGColorpicker
{
	...
	public static function process($clr, $processColor = false)
	{
		if (!is_string($clr)) {
			...
			return [ESGColorpicker::processGradient($clr), 'gradient', $clr];
			...
			return [ESGColorpicker::processGradient($reversed), 'gradient_css', $reversed];
		} elseif (strpos($clr, '#') !== false) {
			return [ESGColorpicker::sanitizeHex($clr), 'hex'];
	...
	public static function sanatizeGradient($obj)
	{
		...
		for ($i = 0; $i < $len; $i++) {
			...
			if (isset($prev)) {
				if (wp_json_encode($cur) !== wp_json_encode($prev)) {
					$ar[] = $cur;
				}
			} else {
				$ar[] = $cur;
	...
	{
		$return = get_option('tp_colorpicker_presets', []);
		if (!is_array($return) || empty($return)) $return = [ 'colors' => [], 'gradients' => [] ];

...
class Essential_Grid_Item_Skin
{
	...
	private int $item_counter = 0; //for custom grids, holds the ID that is needed for search results and to give each Item a unique class
				...
				if (!empty($this->lightbox_additions['items'])) {
					...
					foreach ($this->lightbox_additions['items'] as $lb_key => $lb_img) {
						...
						echo '<a href="' . esc_attr($lb_img) . '" class="esgbox" ' 
						     ...
						     . '>'
						     // phpcs:ignore PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage 
						     . '<img class="esg-lb-dummy" src="' . esc_url($thumg_src) . '" alt="" />' 
						     . '</a>';
					}
					echo '</div>';
				}
	...
	protected function _output_css($css) {
		...
		echo '<style>' . $base->compress_css($css) . '</style>' . "\n";
	}
	...
	public function insert_layer($layer, $demo = false, $masonry = false, $grid_ids = '', $post_ids = '') {
		...
		switch ($link_to) {
			...
			case 'lightbox':
				...
				$text = '<a data-thumb="' . esc_url(esg_aq_resize($this->lightbox_thumbnail, 200)) . '" href="' . esc_url($lb_source) . '"' . $lb_addition . $lb_caption . $lb_owidth . $lb_oheight . $lb_rel . $lb_data . $lb_featured . $lb_post_title . '>' . $text . '</a>';
	...
	public function get_post_value($handle, $separator, $function, $meta, $catmax = '-1', $taxonomy = "") {
		...
		switch ($handle) {
			...
			case 'date_day':
				$postDate = get_date_from_gmt($base->getVar($this->post, "post_date_gmt"));
				$text = gmdate('d', strtotime($postDate));
				break;
			case 'date_month':
				$postDate = get_date_from_gmt($base->getVar($this->post, "post_date_gmt"));
				$text = gmdate('m', strtotime($postDate));
				break;
			...
			case 'date_month_name':
				$postDate = get_date_from_gmt($base->getVar($this->post, "post_date_gmt"));
				$text = gmdate('F', strtotime($postDate));
				break;
			case 'date_year':
				$postDate = get_date_from_gmt($base->getVar($this->post, "post_date_gmt"));
				$text = gmdate('Y', strtotime($postDate));
				break;
			case 'date_year_abbr':
				$postDate = get_date_from_gmt($base->getVar($this->post, "post_date_gmt"));
				$text = gmdate('y', strtotime($postDate));

...
class Essential_Grid_Widget_Areas
{
	...
	{
		if (!isset($new_area['handle']) || strlen($new_area['handle']) < 3) return esc_attr__('Wrong Handle received', 'essential-grid');
		if (!isset($new_area['name']) || strlen($new_area['name']) < 3) return esc_attr__('Wrong Name received', 'essential-grid');

		$sidebars = $this->get_all_sidebars();
		if ($sidebars !== false) {
			foreach ($sidebars as $handle => $name) {
				if ($handle == $new_area['handle']) return esc_attr__('Widget Area with handle already exist, choose a different handle', 'essential-grid');
			}
		}
	...
	{
		if (!isset($edit_widget['handle']) || strlen($edit_widget['handle']) < 3) return esc_attr__('Wrong Handle received', 'essential-grid');
		if (!isset($edit_widget['name']) || strlen($edit_widget['name']) < 3) return esc_attr__('Wrong Name received', 'essential-grid');

		$sidebars = $this->get_all_sidebars();
		if (!$sidebars || !is_array($sidebars)) return esc_attr__('No Ess. Grid Widget Areas exist', 'essential-grid');
		foreach ($sidebars as $handle => $name) {

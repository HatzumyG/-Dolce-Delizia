...
class Essential_Grid_Meta
{
	...
	public function add_new_meta($new_meta)
	{
		...
		if (!isset($new_meta['handle']) || strlen($new_meta['handle']) < 3) return esc_attr__('Wrong Meta Handle received', 'essential-grid');
		if (!isset($new_meta['name']) || strlen($new_meta['name']) < 3) return esc_attr__('Wrong Meta Name received', 'essential-grid');
		if (!isset($new_meta['sort-type'])) $new_meta['sort-type'] = 'alphabetic';

		$metas = $this->get_all_meta(false);
		if (!empty($metas)) {
			foreach ($metas as $meta) {
				if ($meta['handle'] == $new_meta['handle']) return esc_attr__('Meta Handle already exist, choose a different handle', 'essential-grid');
			}
		} else {
			$metas = [];
		}
		...
		if ($new_meta['type'] == 'select' || $new_meta['type'] == 'multi-select') {
			if (!isset($new_meta['sel']) || strlen($new_meta['sel']) < 3) return esc_attr__('Wrong Meta Select received', 'essential-grid');
	...
	public function edit_meta_by_handle($edit_meta)
	{
		...
		if (!isset($edit_meta['handle']) || strlen($edit_meta['handle']) < 3) return esc_attr__('Wrong Meta Handle received', 'essential-grid');
		if (!isset($edit_meta['name']) || strlen($edit_meta['name']) < 3) return esc_attr__('Wrong Meta Name received', 'essential-grid');
					...
					foreach ($matches as $match) {
						...
						if (empty($meta_value) && !empty($my_post)) {
							...
							switch ($meta) {
								...
								case 'date_day':
									$postDate = $base->getVar($my_post, "post_date_gmt");
									$meta_value = gmdate('d', strtotime($postDate));
									break;
								case 'date_month':
									$postDate = $base->getVar($my_post, "post_date_gmt");
									$meta_value = gmdate('m', strtotime($postDate));
									break;
								...
								case 'date_year':
									$postDate = $base->getVar($my_post, "post_date_gmt");
									$meta_value = gmdate('Y', strtotime($postDate));
									break;
								case 'date_year_abbr':
									$postDate = $base->getVar($my_post, "post_date_gmt");
									$meta_value = gmdate('y', strtotime($postDate));
	...
	public function save_all_metas($metas)
	{
		if (!empty($metas)) {
			foreach ($metas as $k => $meta) {
				if (!isset($meta['handle']) || strlen($meta['handle']) < 3) return esc_attr__('Wrong Meta Handle received', 'essential-grid');
				if (preg_replace('/[^a-zA-Z0-9\-_]/', '', $meta['handle']) != $meta['handle']) {
					return sprintf(
						/* translators: %s: meta handle slug */
						esc_attr__('Meta Handle "%s" contain forbidden characters!', 'essential-grid'), 
						$meta['handle'] 
					);
				}
				if (!isset($meta['name']) || strlen($meta['name']) < 3) return esc_attr__('Wrong Meta Name received', 'essential-grid');
				if (!isset($meta['sort-type'])) $metas[$k]['sort-type'] = 'alphabetic';

				if ($meta['type'] == 'select' || $meta['type'] == 'multi-select') {
					if (!isset($meta['select']) || strlen($meta['select']) < 3) return esc_attr__('Wrong Meta Select received', 'essential-grid');
				}
			}
		}

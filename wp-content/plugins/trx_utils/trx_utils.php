...
if ( ! defined( 'TRX_UTILS_VERSION' ) ) {
	define( 'TRX_UTILS_VERSION', '3.2.9' );
}
...
if ( !function_exists( 'trx_utils_core_frontend_scripts' ) ) {
    function trx_utils_core_frontend_scripts() {
        // Google map
        if ( function_exists('sweet_dessert_get_custom_option') && sweet_dessert_get_custom_option('show_googlemap')=='yes' ) {
            ...
            wp_enqueue_script( 'sweet_dessert-googlemap-script', trx_utils_get_file_url('js/core.googlemap.js'), array(), null, true );
        }

        if ( function_exists('sweet_dessert_get_custom_option') && is_single() && sweet_dessert_get_custom_option('show_reviews')=='yes' ) {
			wp_enqueue_script( 'sweet-dessert-core-reviews-script', trx_utils_get_file_url('js/core.reviews.js'), array(), null, true );
        }
    }
}
...
if (!function_exists('sweet_dessert_add_fields_in_user_info')) {
    function sweet_dessert_add_fields_in_user_info( $user ) {
        ?>
            ...
            if (is_array($social_list) && count($social_list) > 0) {
                foreach ($social_list as $soc) {
                    ...
                    if (!empty($sn)) {
                        ?>
                        <tr>
                            <th><label for="user_<?php echo esc_attr($sn); ?>"><?php sweet_dessert_show_layout(sweet_dessert_strtoproper($sn)); ?>:</label></th>
                            <td><input type="text" name="user_<?php echo esc_attr($sn); ?>" id="user_<?php echo esc_attr($sn); ?>" size="55" value="<?php echo esc_attr(get_the_author_meta('user_'.($sn), $user->ID)); ?>" />
                                <span class="description"><?php echo sprintf(esc_html__('Please, enter your %s link', 'trx_utils'), sweet_dessert_strtoproper($sn)); ?></span>
                            </td>
                        </tr>
                        <?php
                    }
...
if (!function_exists('sweet_dessert_save_fields_in_user_info')) {
    function sweet_dessert_save_fields_in_user_info( $user_id ) {
...
if(!function_exists('trx_utils_filter_options')){
    add_filter('sweet_dessert_filter_options', 'trx_utils_filter_options');
...
if (function_exists('sweet_dessert_get_custom_option')) {
    if (sweet_dessert_get_custom_option('show_googlemap') == 'yes' && sweet_dessert_get_theme_option('api_google') != '') {
        ...
        wp_enqueue_script('sweet-dessert-googlemap-script', trx_utils_get_file_url('js/core.googlemap.js'), array(), null, true);
    }
}
...
if (!function_exists('trx_utils_widgets_disable_block_editor')) {
    ...
    function trx_utils_widgets_disable_block_editor() {
        if ( function_exists('sweet_dessert_get_theme_option') && sweet_dessert_get_theme_option( 'disable_widgets_block_editor' ) == 'yes' ) {

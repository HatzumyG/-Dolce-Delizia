<?php
/**
 * Sweet Dessert Framework: Testimonial support
 *
...
// Theme init
if (!function_exists('sweet_dessert_testimonial_theme_setup')) {
	add_action( 'sweet_dessert_action_before_init_theme', 'sweet_dessert_testimonial_theme_setup', 1 );
	function sweet_dessert_testimonial_theme_setup() {
	
		// Add item in the admin menu
		add_filter('trx_utils_filter_override_options',		'sweet_dessert_testimonial_add_override_options');

		// Save data from override options
		add_action('save_post',				'sweet_dessert_testimonial_save_data');

		// Register shortcodes [trx_testimonials] and [trx_testimonials_item]
		add_action('sweet_dessert_action_shortcodes_list',		'sweet_dessert_testimonials_reg_shortcodes');
		if (function_exists('sweet_dessert_exists_visual_composer') && sweet_dessert_exists_visual_composer())
			add_action('sweet_dessert_action_shortcodes_list_vc','sweet_dessert_testimonials_reg_shortcodes_vc');

		// Override options fields
		sweet_dessert_storage_set('testimonial_override_options', array(
		...
		sweet_dessert_theme_support_pt('testimonial');
		sweet_dessert_theme_support_tx('testimonial_group');
...
if (!function_exists('sweet_dessert_testimonial_add_override_options')) {
	function sweet_dessert_testimonial_add_override_options($boxes = array()) {
        $boxes[] = array_merge(sweet_dessert_storage_get('testimonial_override_options'), array('callback' => 'sweet_dessert_testimonial_show_override_options'));
        return $boxes;
	}
}

// Callback function to show fields in override options
if (!function_exists('sweet_dessert_testimonial_show_override_options')) {
	function sweet_dessert_testimonial_show_override_options() {
		global $post;

		// Use nonce for verification
		echo '<input type="hidden" name="override_options_testimonial_nonce" value="'.esc_attr(wp_create_nonce(admin_url())).'" />';
		
		$data = get_post_meta($post->ID, sweet_dessert_storage_get('options_prefix').'_testimonial_data', true);
	
		$fields = sweet_dessert_storage_get_array('testimonial_override_options', 'fields');
		?>
...
// Save data from override options
if (!function_exists('sweet_dessert_testimonial_save_data')) {
	function sweet_dessert_testimonial_save_data($post_id) {
		// verify nonce
		if ( !wp_verify_nonce( sweet_dessert_get_value_gp('override_options_testimonial_nonce'), admin_url() ) )
			return $post_id;
		...
		update_post_meta($post_id, sweet_dessert_storage_get('options_prefix').'_testimonial_data', $data);
	}
}
...
if (!function_exists('sweet_dessert_sc_testimonials')) {
	function sweet_dessert_sc_testimonials($atts, $content=null){	
		...
		sweet_dessert_storage_set('sc_testimonials_data', array(
			'id' => $id,
            'style' => $style,
            'columns' => $columns,
            'counter' => 0,
		...
		$output = ($bg_color!='' || $bg_image!='' || $bg_overlay>0 || $bg_texture>0 || sweet_dessert_strlen($bg_texture)>2 || ($scheme && !sweet_dessert_param_is_off($scheme) && !sweet_dessert_param_is_inherit($scheme))
				...
				. ' class="sc_testimonials sc_testimonials_style_'.esc_attr($style)
					...
					. '"'
				. ($bg_color=='' && $bg_image=='' && $bg_overlay==0 && ($bg_texture=='' || $bg_texture=='0') && !sweet_dessert_param_is_off($animation) ? ' data-animation="'.esc_attr(sweet_dessert_get_animation_classes($animation)).'"' : '')
				. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
			. '>'
			. (!empty($subtitle) ? '<h6 class="sc_testimonials_subtitle sc_item_subtitle">' . trim(sweet_dessert_strmacros($subtitle)) . '</h6>' : '')
			. (!empty($title) ? '<h2 class="sc_testimonials_title sc_item_title' . (empty($description) ? ' sc_item_title_without_descr' : ' sc_item_title_with_descr') . '">' . trim(sweet_dessert_strmacros($title)) . '</h2>' : '')
			. (!empty($description) ? '<div class="sc_testimonials_descr sc_item_descr">' . trim(sweet_dessert_strmacros($description)) . '</div>' : '')
		...
		if (sweet_dessert_param_is_on($custom) && $content) {
			...
			$args = sweet_dessert_query_add_posts_and_cats($args, $ids, 'testimonial', $cat, 'testimonial_group');
			...
			while ( $query->have_posts() ) { 
				...
				$post_data = sweet_dessert_get_post_data($args);
				...
				$post_meta = get_post_meta($post_data['post_id'], sweet_dessert_storage_get('options_prefix').'_testimonial_data', true);
				$thumb_sizes = sweet_dessert_get_thumb_sizes(array('layout' => $style));
		...
		$output .= '</div>'
					. ($bg_color!='' || $bg_image!='' || $bg_overlay>0 || $bg_texture>0 || sweet_dessert_strlen($bg_texture)>2 || ($scheme && !sweet_dessert_param_is_off($scheme) && !sweet_dessert_param_is_inherit($scheme))
						?  '</div></div>'
						: '');
		...
		return apply_filters('sweet_dessert_shortcode_output', $output, 'trx_testimonials', $atts, $content);
	}
	add_shortcode('trx_testimonials', 'sweet_dessert_sc_testimonials');
}
	
	
if (!function_exists('sweet_dessert_sc_testimonials_item')) {
	function sweet_dessert_sc_testimonials_item($atts, $content=null){	
		if (sweet_dessert_in_shortcode_blogger()) return '';
		extract(sweet_dessert_html_decode(shortcode_atts(array(
			// Individual params
			"author" => "",
		...
		sweet_dessert_storage_inc_array('sc_testimonials_data', 'counter');
	
		$id = $id ? $id : (sweet_dessert_storage_get_array('sc_testimonials_data', 'id') ? sweet_dessert_storage_get_array('sc_testimonials_data', 'id') . '_' . sweet_dessert_storage_get_array('sc_testimonials_data', 'counter') : '');
	
		$thumb_sizes = sweet_dessert_get_thumb_sizes(array('layout' => sweet_dessert_storage_get_array('sc_testimonials_data', 'style')));
		...
		if (empty($photo)) {
			...
			$photo = sweet_dessert_get_resized_image_tag($photo, $thumb_sizes['w'], $thumb_sizes['h']);
		}
		...
		$args = array(
			'layout' => sweet_dessert_storage_get_array('sc_testimonials_data', 'style'),
			'number' => sweet_dessert_storage_get_array('sc_testimonials_data', 'counter'),
			'columns_count' => sweet_dessert_storage_get_array('sc_testimonials_data', 'columns'),
			'slider' => sweet_dessert_storage_get_array('sc_testimonials_data', 'slider'),
			'show' => false,
			'descr'  => 0,
			'tag_id' => $id,
			'tag_class' => $class,
			'tag_animation' => '',
			'tag_css' => $css,
			'tag_css_wh' => sweet_dessert_storage_get_array('sc_testimonials_data', 'css_wh'),
			'author' => $author,
			'position' => $position,
			'link' => $link,
			'email' => $email,
		...
		return apply_filters('sweet_dessert_shortcode_output', $output, 'trx_testimonials_item', $atts, $content);
	}
	add_shortcode('trx_testimonials_item', 'sweet_dessert_sc_testimonials_item');
}
// ---------------------------------- [/trx_testimonials] ---------------------------------------
...
if (!function_exists('sweet_dessert_testimonials_reg_shortcodes')) {
	function sweet_dessert_testimonials_reg_shortcodes() {
		if (sweet_dessert_storage_isset('shortcodes')) {

			$testimonials_groups = sweet_dessert_get_list_terms(false, 'testimonial_group');
			$testimonials_styles = sweet_dessert_get_list_templates('testimonials');
			$controls = sweet_dessert_get_list_slider_controls();

			sweet_dessert_sc_map_before('trx_title', array(
				...
				"trx_testimonials" => array(
					...
					"params" => array(
						...
						"cat" => array(
							...
							"style" => "list",
							"multiple" => true,
							"options" => sweet_dessert_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), $testimonials_groups)
						),
						"count" => array(
...
if (!function_exists('sweet_dessert_testimonials_reg_shortcodes_vc')) {
	function sweet_dessert_testimonials_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"params" => array(
					...
					array(
						...
						"class" => "",
						"value" => array_flip(sweet_dessert_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), $testimonials_groups)),
						"type" => "dropdown"
					),
					array(
						"param_name" => "columns",
		...
		class WPBakeryShortCode_Trx_Testimonials extends Sweet_Dessert_VC_ShortCodeColumns {}
		class WPBakeryShortCode_Trx_Testimonials_Item extends Sweet_Dessert_VC_ShortCodeCollection {}

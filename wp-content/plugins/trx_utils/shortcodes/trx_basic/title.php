...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('sweet_dessert_sc_title_theme_setup')) {
	add_action( 'sweet_dessert_action_before_init_theme', 'sweet_dessert_sc_title_theme_setup' );
	function sweet_dessert_sc_title_theme_setup() {
		add_action('sweet_dessert_action_shortcodes_list', 		'sweet_dessert_sc_title_reg_shortcodes');
		if (function_exists('sweet_dessert_exists_visual_composer') && sweet_dessert_exists_visual_composer())
			add_action('sweet_dessert_action_shortcodes_list_vc','sweet_dessert_sc_title_reg_shortcodes_vc');
	}
}
...
if (!function_exists('sweet_dessert_sc_title')) {	
	function sweet_dessert_sc_title($atts, $content=null){	
		...
		$css .= sweet_dessert_get_css_dimensions_from_values($width)
			.($align && $align!='none' && !sweet_dessert_param_is_inherit($align) ? 'text-align:' . esc_attr($align) .';' : '')
		...
		$pic = $style!='iconed' 
			...
			: '<span class="sc_title_icon sc_title_icon_'.esc_attr($position).'  sc_title_icon_'.esc_attr($image_size).($icon!='' && $icon!='none' ? ' '.esc_attr($icon) : '').'"'.'>'
				...
				.(empty($picture) && $image && $image!='none' ? '<img src="'.esc_url(sweet_dessert_strpos($image, 'http')===0 ? $image : sweet_dessert_get_file_url('images/icons/'.($image).'.png')).'" alt="" />' : '')
				.'</span>';
		$output = '<h' . esc_attr($type) . ($id ? ' id="'.esc_attr($id).'"' : '')
				. ' class="sc_title sc_title_'.esc_attr($style)
					.($align && $align!='none' && !sweet_dessert_param_is_inherit($align) ? ' sc_align_' . esc_attr($align) : '')
				...
				. '</h' . esc_attr($type) . '>';
		return apply_filters('sweet_dessert_shortcode_output', $output, 'trx_title', $atts, $content);
	}
	add_shortcode('trx_title', 'sweet_dessert_sc_title');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_title_reg_shortcodes' ) ) {
	function sweet_dessert_sc_title_reg_shortcodes() {
	
		sweet_dessert_sc_map("trx_title", array(
			...
			"params" => array(
				...
				"image" => array(
					...
					"type" => "images",
					"size" => "small",
					"options" => sweet_dessert_get_sc_param('images')
				),
				"picture" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_title_reg_shortcodes_vc' ) ) {
	function sweet_dessert_sc_title_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				...
				array(
					...
					"value" => sweet_dessert_get_sc_param('images'),
					"type" => "dropdown"
				),
				array(
					"param_name" => "picture",
		...
		class WPBakeryShortCode_Trx_Title extends Sweet_Dessert_VC_ShortCodeSingle {}
	}
}
?>
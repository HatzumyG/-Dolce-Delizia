...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('sweet_dessert_sc_section_theme_setup')) {
	add_action( 'sweet_dessert_action_before_init_theme', 'sweet_dessert_sc_section_theme_setup' );
	function sweet_dessert_sc_section_theme_setup() {
		add_action('sweet_dessert_action_shortcodes_list', 		'sweet_dessert_sc_section_reg_shortcodes');
		if (function_exists('sweet_dessert_exists_visual_composer') && sweet_dessert_exists_visual_composer())
			add_action('sweet_dessert_action_shortcodes_list_vc','sweet_dessert_sc_section_reg_shortcodes_vc');
	}
}
...
if (!function_exists('sweet_dessert_sc_section')) {	
	function sweet_dessert_sc_section($atts, $content=null){	
		...
		$css .= ($color !== '' ? 'color:' . esc_attr($color) . ';' : '')
			...
			.($bg_image !== '' ? 'background-image:url(' . esc_url($bg_image) . ');'.(sweet_dessert_param_is_on($bg_tile) ? 'background-repeat:repeat;' : 'background-repeat:no-repeat;background-size:cover;') : '')
			.(!sweet_dessert_param_is_off($pan) ? 'position:relative;' : '')
			.($font_size != '' ? 'font-size:' . esc_attr(sweet_dessert_prepare_css_value($font_size)) . '; line-height: 1.3em;' : '')
			.($font_weight != '' && !sweet_dessert_param_is_inherit($font_weight) ? 'font-weight:' . esc_attr($font_weight) . ';' : '');
		$css_dim = sweet_dessert_get_css_dimensions_from_values($width, $height);
		if ($bg_image == '' && $bg_color == '' && $bg_overlay==0 && $bg_texture==0 && sweet_dessert_strlen($bg_texture)<2) $css .= $css_dim;
		
		$width  = sweet_dessert_prepare_css_value($width);
		$height = sweet_dessert_prepare_css_value($height);
	
		if ((!sweet_dessert_param_is_off($scroll) || !sweet_dessert_param_is_off($pan)) && empty($id)) $id = 'sc_section_'.str_replace('.', '', mt_rand());
	
		if (!sweet_dessert_param_is_off($scroll)) sweet_dessert_enqueue_slider();
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="sc_section' 
					...
					. (sweet_dessert_param_is_on($scroll) && !sweet_dessert_param_is_off($scroll_controls) ? ' sc_scroll_controls sc_scroll_controls_'.esc_attr($scroll_dir).' sc_scroll_controls_type_'.esc_attr($scroll_controls) : '')
					. '"'
				...
				. '<div class="sc_section_inner">'
					. ($bg_image !== '' || $bg_color !== '' || $bg_overlay>0 || $bg_texture>0 || sweet_dessert_strlen($bg_texture)>2
						? '<div class="sc_section_overlay'.($bg_texture>0 ? ' texture_bg_'.esc_attr($bg_texture) : '') . '"'
							. ' style="' . ($bg_overlay>0 ? 'background-color:rgba('.(int)$rgb['r'].','.(int)$rgb['g'].','.(int)$rgb['b'].','.min(1, max(0, $bg_overlay)).');' : '')
								...
								. '>'
								. '<div class="sc_section_content' . (sweet_dessert_param_is_on($bg_padding) ? ' padding_on' : ' padding_off') . '"'
					...
					. (sweet_dessert_param_is_on($scroll) 
						...
						: '')
					. (sweet_dessert_param_is_on($pan) 
						? '<div id="'.esc_attr($id).'_pan" class="sc_pan sc_pan_'.esc_attr($scroll_dir).'">' 
						: '')
							. (!empty($subtitle) ? '<h6 class="sc_section_subtitle sc_item_subtitle">' . trim(sweet_dessert_strmacros($subtitle)) . '</h6>' : '')
							. (!empty($title) ? '<h4 class="sc_section_title sc_item_title' . (empty($description) ? ' sc_item_title_without_descr' : ' sc_item_title_without_descr') . '">' . trim(sweet_dessert_strmacros($title)) . '</h4>' : '')
							. (!empty($description) ? '<div class="sc_section_descr sc_item_descr">' . trim(sweet_dessert_strmacros($description)) . '</div>' : '')
							. '<div class="sc_section_content_wrap">' . do_shortcode($content) . '</div>'
							. (!empty($link) ? '<div class="sc_section_button sc_item_button">'.sweet_dessert_do_shortcode('[trx_button link="'.esc_url($link).'"]'.esc_html($link_caption).'[/trx_button]').'</div>' : '')
					. (sweet_dessert_param_is_on($pan) ? '</div>' : '')
					. (sweet_dessert_param_is_on($scroll) 
						? '</div></div><div id="'.esc_attr($id).'_scroll_bar" class="sc_scroll_bar sc_scroll_bar_'.esc_attr($scroll_dir).' '.esc_attr($id).'_scroll_bar"></div></div>'
							. (!sweet_dessert_param_is_off($scroll_controls) ? '<div class="sc_scroll_controls_wrap"><a class="sc_scroll_prev" href="#"></a><a class="sc_scroll_next" href="#"></a></div>' : '')
						: '')
					. ($bg_image !== '' || $bg_color !== '' || $bg_overlay > 0 || $bg_texture>0 || sweet_dessert_strlen($bg_texture)>2 ? '</div></div>' : '')
					. '</div>'
				. '</div>';
		if (sweet_dessert_param_is_on($dedicated)) {
			if (sweet_dessert_storage_get('sc_section_dedicated')=='') {
				sweet_dessert_storage_set('sc_section_dedicated', $output);
			}
			$output = '';
		}
		return apply_filters('sweet_dessert_shortcode_output', $output, 'trx_section', $atts, $content);
	}
	add_shortcode('trx_section', 'sweet_dessert_sc_section');
}

if (!function_exists('sweet_dessert_sc_block')) {	
	function sweet_dessert_sc_block($atts, $content=null) {
		$atts['class'] = (!empty($atts['class']) ? $atts['class'] . ' ' : '') . 'sc_section_block';
		return apply_filters('sweet_dessert_shortcode_output', sweet_dessert_sc_section($atts, $content), 'trx_block', $atts, $content);
	}
	add_shortcode('trx_block', 'sweet_dessert_sc_block');
}


/* Register shortcode in the internal SC Builder
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_section_reg_shortcodes' ) ) {
	function sweet_dessert_sc_section_reg_shortcodes() {
		...
		$sc = array(
			...
			"params" => array(
				...
				"columns" => array(
					...
					"value" => "none",
					"type" => "checklist",
					"options" => sweet_dessert_get_sc_param('columns')
				), 
				"pan" => array(
				...
				"scroll_controls" => array(
					...
					"value" => "hide",
					"type" => "checklist",
					"options" => sweet_dessert_get_sc_param('controls')
				),
				"scheme" => array(
		...
		sweet_dessert_sc_map("trx_block", $sc);
		...
		sweet_dessert_sc_map("trx_section", $sc);
	}
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_section_reg_shortcodes_vc' ) ) {
	function sweet_dessert_sc_section_reg_shortcodes_vc() {
			...
			"params" => array(
                ...
                array(
                    ...
                    "value" => array(
                        ...
                        esc_html__('Original with small margin before description', 'trx_utils') => 'original_margin',
                        esc_html__('Original with small margin before title', 'trx_utils') => 'original_smallmargin',
                        esc_html__('Promo section', 'trx_utils') => 'promo_section',
                        esc_html__('Promo blog section', 'trx_utils') => 'promo_blog_section'
                    ),
                    "type" => "dropdown"
                ),
				array(
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip(sweet_dessert_get_sc_param('columns')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "title",
				...
				array(
					...
					"value" => array_flip(sweet_dessert_get_sc_param('controls')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "scheme",
		...
		class WPBakeryShortCode_Trx_Block extends Sweet_Dessert_VC_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_Section extends Sweet_Dessert_VC_ShortCodeCollection {}
	}
}
?>
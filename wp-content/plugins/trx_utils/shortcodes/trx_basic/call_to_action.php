...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('sweet_dessert_sc_call_to_action_theme_setup')) {
	add_action( 'sweet_dessert_action_before_init_theme', 'sweet_dessert_sc_call_to_action_theme_setup' );
	function sweet_dessert_sc_call_to_action_theme_setup() {
		add_action('sweet_dessert_action_shortcodes_list', 		'sweet_dessert_sc_call_to_action_reg_shortcodes');
		if (function_exists('sweet_dessert_exists_visual_composer') && sweet_dessert_exists_visual_composer())
			add_action('sweet_dessert_action_shortcodes_list_vc','sweet_dessert_sc_call_to_action_reg_shortcodes_vc');
	}
}
...
if (!function_exists('sweet_dessert_sc_call_to_action')) {	
	function sweet_dessert_sc_call_to_action($atts, $content=null){	
		...
		if (!empty($image)) {
			$thumb_sizes = sweet_dessert_get_thumb_sizes(array('layout' => 'excerpt'));
			$image = !empty($video) 
				? sweet_dessert_get_resized_image_url($image, $thumb_sizes['w'], $thumb_sizes['h']) 
				: sweet_dessert_get_resized_image_tag($image, $thumb_sizes['w'], $thumb_sizes['h']);
		}
	
		if (!empty($video)) {
			$video = '<video' . ($id ? ' id="' . esc_attr($id.'_video') . '"' : '') 
				. ' class="sc_video"'
				. ' src="' . esc_url(sweet_dessert_get_video_player_url($video)) . '"'
				...
				. '>'
				. '</video>';
			if (sweet_dessert_get_custom_option('substitute_video')=='no') {
				$video = sweet_dessert_get_video_frame($video, $image, '', '');
			} else {
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="sc_call_to_action'
					. (sweet_dessert_param_is_on($accent) ? ' sc_call_to_action_accented' : '')
			...
			. '>'
				...
				. '<div class="sc_call_to_action_info'.($need_columns ? ' column-'.esc_attr($need_columns-1).'_'.esc_attr($need_columns) : '').'">'
					. (!empty($subtitle) ? '<h6 class="sc_call_to_action_subtitle sc_item_subtitle">' . trim(sweet_dessert_strmacros($subtitle)) . '</h6>' : '')
					. (!empty($title) ? '<h2 class="sc_call_to_action_title sc_item_title' . (empty($description) ? ' sc_item_title_without_descr' : ' sc_item_title_with_descr') . '">' . trim(sweet_dessert_strmacros($title)) . '</h2>' : '')
					. (!empty($description) ? '<div class="sc_call_to_action_descr sc_item_descr">' . trim(sweet_dessert_strmacros($description)) . '</div>' : '')
					. ($style==1 ? $buttons : '')
				. '</div>'
				. ($style==2 && $align!='right' ? $buttons : '')
				. ($align=='right' ? $featured : '')
		...
		return apply_filters('sweet_dessert_shortcode_output', $output, 'trx_call_to_action', $atts, $content);
	}
	add_shortcode('trx_call_to_action', 'sweet_dessert_sc_call_to_action');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_call_to_action_reg_shortcodes' ) ) {
	function sweet_dessert_sc_call_to_action_reg_shortcodes() {
	
		sweet_dessert_sc_map("trx_call_to_action", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_call_to_action_reg_shortcodes_vc' ) ) {
	function sweet_dessert_sc_call_to_action_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Call_To_Action extends Sweet_Dessert_VC_ShortCodeCollection {}
	}
}
?>
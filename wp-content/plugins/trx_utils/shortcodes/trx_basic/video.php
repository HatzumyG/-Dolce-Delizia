...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('sweet_dessert_sc_video_theme_setup')) {
	add_action( 'sweet_dessert_action_before_init_theme', 'sweet_dessert_sc_video_theme_setup' );
	function sweet_dessert_sc_video_theme_setup() {
		add_action('sweet_dessert_action_shortcodes_list', 		'sweet_dessert_sc_video_reg_shortcodes');
		if (function_exists('sweet_dessert_exists_visual_composer') && sweet_dessert_exists_visual_composer())
			add_action('sweet_dessert_action_shortcodes_list_vc','sweet_dessert_sc_video_reg_shortcodes_vc');
	}
}
...
if (!function_exists('sweet_dessert_sc_video')) {	
	function sweet_dessert_sc_video($atts, $content = null) {
		...
		if (empty($height) && empty($width)) {
			$width='100%';
			if (sweet_dessert_param_is_off(sweet_dessert_get_custom_option('substitute_video'))) $height="400";
		}
		$ed = sweet_dessert_substr($width, -1);
		...
		$url = $src!='' ? $src : $url;
		if ($image!='' && sweet_dessert_param_is_off($image))
			$image = '';
		else {
			if (sweet_dessert_param_is_on($autoplay) && is_singular() && !sweet_dessert_storage_get('blog_streampage'))
				$image = '';
			else {
				if ($image > 0) {
				...
				if ($bg_image) {
					...
					if (!is_single() || !empty($image)) $image = sweet_dessert_get_resized_image_url(empty($image) ? get_the_ID() : $image, $thumb_sizes['w'], $thumb_sizes['h'], null, false, false, false);
				} else
					if (!is_single() || !empty($image)) $image = sweet_dessert_get_resized_image_url(empty($image) ? get_the_ID() : $image, $ed!='%' ? $width : null, $height);
				if (empty($image) && (!is_singular() || sweet_dessert_storage_get('blog_streampage')))	// || sweet_dessert_param_is_off($autoplay)))
					$image = sweet_dessert_get_video_cover_image($url);
			}
		}
		if ($bg_image > 0) {
		...
		$url = sweet_dessert_get_video_player_url($src!='' ? $src : $url);
		...
		$video = '<video' . ($id ? ' id="' . esc_attr($id) . '"' : '') 
			...
			. (($image && sweet_dessert_param_is_on(sweet_dessert_get_custom_option('substitute_video'))) || (sweet_dessert_param_is_on($autoplay) && is_singular() && !sweet_dessert_storage_get('blog_streampage')) ? ' autoplay="autoplay"' : '') 
			. ' controls="controls" loop="loop"'
			. '>'
			. '</video>';
		if (sweet_dessert_param_is_off(sweet_dessert_get_custom_option('substitute_video'))) {
			if (sweet_dessert_param_is_on($frame)) $video = sweet_dessert_get_video_frame($video, $image, $css, $css_bg);
		} else {
			if ((isset($_GET['vc_editable']) && $_GET['vc_editable']=='true') && (isset($_POST['action']) && $_POST['action']=='vc_load_shortcode')) {
				$video = sweet_dessert_substitute_video($video, $width, $height, false);
			}
		}
		if (sweet_dessert_get_theme_option('use_mediaelement')=='yes')
			wp_enqueue_script('wp-mediaelement');
		return apply_filters('sweet_dessert_shortcode_output', $video, 'trx_video', $atts, $content);
	}
	add_shortcode("trx_video", "sweet_dessert_sc_video");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_video_reg_shortcodes' ) ) {
	function sweet_dessert_sc_video_reg_shortcodes() {
	
		sweet_dessert_sc_map("trx_video", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_video_reg_shortcodes_vc' ) ) {
	function sweet_dessert_sc_video_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Video extends Sweet_Dessert_VC_ShortCodeSingle {}
	}
}
?>
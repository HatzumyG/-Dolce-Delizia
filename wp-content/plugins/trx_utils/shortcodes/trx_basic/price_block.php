...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('sweet_dessert_sc_price_block_theme_setup')) {
	add_action( 'sweet_dessert_action_before_init_theme', 'sweet_dessert_sc_price_block_theme_setup' );
	function sweet_dessert_sc_price_block_theme_setup() {
		add_action('sweet_dessert_action_shortcodes_list', 		'sweet_dessert_sc_price_block_reg_shortcodes');
		if (function_exists('sweet_dessert_exists_visual_composer') && sweet_dessert_exists_visual_composer())
			add_action('sweet_dessert_action_shortcodes_list_vc','sweet_dessert_sc_price_block_reg_shortcodes_vc');
	}
}
...
if (!function_exists('sweet_dessert_sc_price_block')) {	
	function sweet_dessert_sc_price_block($atts, $content=null){	
		...
		$content = do_shortcode(sweet_dessert_sc_clear_around($content));
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
					...
					. '>'
                    . ($bg_image !== '' ? '<div class="price_image"><img src="'. esc_url($bg_image) .'" alt=""></div>' : '')
			...
			. '</div>';
		return apply_filters('sweet_dessert_shortcode_output', $output, 'trx_price_block', $atts, $content);
	}
	add_shortcode('trx_price_block', 'sweet_dessert_sc_price_block');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_price_block_reg_shortcodes' ) ) {
	function sweet_dessert_sc_price_block_reg_shortcodes() {
	
		sweet_dessert_sc_map("trx_price_block", array(
			...
			"params" => array(
				"style" => array(
					...
					"value" => 1,
					"options" => sweet_dessert_get_list_styles(1, 3),
					"type" => "checklist"
				),
				"title" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_price_block_reg_shortcodes_vc' ) ) {
	function sweet_dessert_sc_price_block_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"class" => "",
					"std" => 1,
					"value" => array_flip(sweet_dessert_get_list_styles(1, 3)),
					"type" => "dropdown"
				),
                array(
                    "param_name" => "bg_image",
                    "heading" => esc_html__("Image URL", 'trx_utils'),
                    "description" => wp_kses_data( __("Select image from library for this section", 'trx_utils') ),
                    "class" => "",
                    "value" => "",
                    "type" => "attach_image"
                ),
		...
		class WPBakeryShortCode_Trx_PriceBlock extends Sweet_Dessert_VC_ShortCodeSingle {}
	}
}
?>
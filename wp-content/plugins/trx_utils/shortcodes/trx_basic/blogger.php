...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('sweet_dessert_sc_blogger_theme_setup')) {
	add_action( 'sweet_dessert_action_before_init_theme', 'sweet_dessert_sc_blogger_theme_setup' );
	function sweet_dessert_sc_blogger_theme_setup() {
		add_action('sweet_dessert_action_shortcodes_list', 		'sweet_dessert_sc_blogger_reg_shortcodes');
		if (function_exists('sweet_dessert_exists_visual_composer') && sweet_dessert_exists_visual_composer())
			add_action('sweet_dessert_action_shortcodes_list_vc','sweet_dessert_sc_blogger_reg_shortcodes_vc');
	}
}
...
*/
sweet_dessert_storage_set('sc_blogger_busy', false);

if (!function_exists('sweet_dessert_sc_blogger')) {	
	function sweet_dessert_sc_blogger($atts, $content=null){	
		if (sweet_dessert_in_shortcode_blogger(true)) return '';
		extract(sweet_dessert_html_decode(shortcode_atts(array(
			...
			"location" => "default",
			"dir" => "horizontal",
			"hover" => sweet_dessert_get_theme_option('hover_style'),
			"hover_dir" => sweet_dessert_get_theme_option('hover_dir'),
			"scroll" => "no",
			"controls" => "no",
			"rating" => "no",
			"info" => "yes",
		...
		sweet_dessert_storage_set('sc_blogger_busy', true);
		sweet_dessert_storage_set('sc_blogger_counter', 0);
		...
		if ($descr == '') $descr = sweet_dessert_get_custom_option('post_excerpt_maxlength'.($columns > 1 ? '_masonry' : ''));
	
		if (!sweet_dessert_param_is_off($scroll)) {
			sweet_dessert_enqueue_slider();
			if (empty($id)) $id = 'sc_blogger_'.str_replace('.', '', mt_rand());
		}
		
		$class = apply_filters('sweet_dessert_filter_blog_class',
					'sc_blogger'
					. ' layout_'.esc_attr($style)
					. ' template_'.esc_attr(sweet_dessert_get_template_name($style))
					. (!empty($class) ? ' '.esc_attr($class) : '')
					. ' ' . esc_attr(sweet_dessert_get_template_property($style, 'container_classes'))
					. ' sc_blogger_' . ($dir=='vertical' ? 'vertical' : 'horizontal')
					. (sweet_dessert_param_is_on($scroll) && sweet_dessert_param_is_on($controls) ? ' sc_scroll_controls sc_scroll_controls_type_top sc_scroll_controls_'.esc_attr($dir) : '')
					. ($descr == 0 ? ' no_description' : ''),
					array('style'=>$style, 'dir'=>$dir, 'descr'=>$descr)
		);
	
		$container = apply_filters('sweet_dessert_filter_blog_container', sweet_dessert_get_template_property($style, 'container'), array('style'=>$style, 'dir'=>$dir));
		...
		$container2 = apply_filters('sweet_dessert_filter_blog_container2', sweet_dessert_get_template_property($style, 'container2'), array('style'=>$style, 'dir'=>$dir));
		...
		$output = '<div'
			...
			. '>'
			. ($container_start)
			. (!empty($subtitle) ? '<h6 class="sc_blogger_subtitle sc_item_subtitle">' . trim(sweet_dessert_strmacros($subtitle)) . '</h6>' : '')
			. (!empty($title) ? '<h2 class="sc_blogger_title sc_item_title' . (empty($description) ? ' sc_item_title_without_descr' : ' sc_item_title_with_descr') . '">' . trim(sweet_dessert_strmacros($title)) . '</h2>' : '')
			. (!empty($description) ? '<div class="sc_blogger_descr sc_item_descr">' . trim(sweet_dessert_strmacros($description)) . '</div>' : '')
			. ($container2_start)
			. ($style=='list' ? '<ul class="sc_list sc_list_style_iconed">' : '')
			. ($dir=='horizontal' && $columns > 1 && sweet_dessert_get_template_property($style, 'need_columns') ? '<div class="columns_wrap">' : '')
			. (sweet_dessert_param_is_on($scroll) 
		...
		if (sweet_dessert_get_template_property($style, 'need_isotope')) {
			if (!sweet_dessert_param_is_off($filters))
				$output .= '<div class="isotope_filters"></div>';
			if ($columns<1) $columns = sweet_dessert_substr($style, -1);
		...
		if (!sweet_dessert_param_is_off($only)) $args = sweet_dessert_query_add_filters($args, $only);
		...
		while ( $query->have_posts() ) { $query->the_post();
			...
			sweet_dessert_storage_inc('sc_blogger_counter');
	
			$args = array(
				'layout' => $style,
				'show' => false,
				'number' => sweet_dessert_storage_get('sc_blogger_counter'),
				'add_view_more' => false,
				...
				"readmore" => $readmore,
				"loadmore" => $loadmore,
				"reviews" => sweet_dessert_param_is_on($rating),
				"dir" => $dir,
				"scroll" => sweet_dessert_param_is_on($scroll),
				"info" => sweet_dessert_param_is_on($info),
				...
				// Get post data
				'strip_teaser' => false,
				'content' => sweet_dessert_get_template_property($style, 'need_content'),
				'terms_list' => !sweet_dessert_param_is_off($filters) || sweet_dessert_get_template_property($style, 'need_terms'),
				'filters' => sweet_dessert_param_is_off($filters) ? '' : $filters,
				'hover' => $hover,
				'hover_dir' => $hover_dir
			);
		...
		if (!sweet_dessert_param_is_off($filters)) {
			...
			if ($filters == 'categories') {			// Use categories as filter items
				...
				$portfolio_parent = $cat ? max(0, sweet_dessert_get_parent_taxonomy_by_property($cat, 'show_filters', 'yes', true, $taxonomy)) : 0;
				$args2 = array(
					'type'			=> $post_type,
		...
		$output	.= (sweet_dessert_param_is_on($scroll) 
				? '</div></div><div id="'.esc_attr($id).'_scroll_bar" class="sc_scroll_bar sc_scroll_bar_'.esc_attr($dir).' '.esc_attr($id).'_scroll_bar"></div></div>'
					. (!sweet_dessert_param_is_off($controls) ? '<div class="sc_scroll_controls_wrap"><a class="sc_scroll_prev" href="#"></a><a class="sc_scroll_next" href="#"></a></div>' : '')
				: '')
			. ($dir=='horizontal' && $columns > 1 && sweet_dessert_get_template_property($style, 'need_columns') ? '</div>' :  '')
			. ($style == 'list' ? '</ul>' : '')
			. ($container2_end)
			. (!empty($link) 
		...
		return apply_filters('sweet_dessert_shortcode_output', $output, 'trx_blogger', $atts, $content);
	}
	add_shortcode('trx_blogger', 'sweet_dessert_sc_blogger');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_blogger_reg_shortcodes' ) ) {
	function sweet_dessert_sc_blogger_reg_shortcodes() {
	
		sweet_dessert_sc_map("trx_blogger", array(
			...
			"params" => array(
				...
				"style" => array(
					...
					"value" => "accordion",
					"type" => "select",
					"options" => sweet_dessert_get_sc_param('blogger_styles')
				),
				"filters" => array(
					...
					"dir" => "horizontal",
					"type" => "checklist",
					"options" => sweet_dessert_get_sc_param('filters')
				),
				"hover" => array(
					...
					"value" => "",
					"type" => "select",
					"options" => sweet_dessert_get_sc_param('hovers')
				),
				"hover_dir" => array(
					...
					"value" => "left_to_right",
					"type" => "select",
					"options" => sweet_dessert_get_sc_param('hovers_dir')
				),
				"dir" => array(
				...
				"post_type" => array(
					...
					"value" => "post",
					"type" => "select",
					"options" => sweet_dessert_get_sc_param('posts_types')
				),
				"ids" => array(
				...
				"only" => array(
					...
					"value" => "no",
					"type" => "select",
					"options" => sweet_dessert_get_sc_param('formats')
				),
				"scroll" => array(
				...
				"location" => array(
					...
					"value" => "default",
					"type" => "select",
					"options" => sweet_dessert_get_sc_param('locations')
				),
				"rating" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_blogger_reg_shortcodes_vc' ) ) {
	function sweet_dessert_sc_blogger_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"std" => "accordion",
					"class" => "",
					"value" => array_flip(sweet_dessert_get_sc_param('blogger_styles')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "filters",
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip(sweet_dessert_get_sc_param('filters')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "hover",
					...
					"class" => "",
					"value" => array_flip(sweet_dessert_get_sc_param('hovers')),
					'dependency' => array(
						'element' => 'style',
				...
				array(
					...
					"class" => "",
					"value" => array_flip(sweet_dessert_get_sc_param('hovers_dir')),
					'dependency' => array(
						'element' => 'style',
						'value' => array('portfolio_2','portfolio_3','portfolio_4','grid_2','grid_3','grid_4','square_2','square_3','square_4','short_2','short_3','short_4','colored_2','colored_3','colored_4')
					),
				...
				array(
					...
					"value" => array_flip(sweet_dessert_get_sc_param('locations')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "dir",
				...
				array(
					...
					"class" => "",
					"value" => array_flip(sweet_dessert_get_sc_param('posts_types')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "ids",
				...
				array(
					...
					"class" => "",
					"group" => esc_html__('Query', 'trx_utils'),
					"value" => array_flip(sweet_dessert_get_sc_param('formats')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "scroll",
		...
		class WPBakeryShortCode_Trx_Blogger extends Sweet_Dessert_VC_ShortCodeSingle {}
	}
}
?>
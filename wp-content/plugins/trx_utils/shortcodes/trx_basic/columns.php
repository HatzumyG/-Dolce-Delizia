...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('sweet_dessert_sc_columns_theme_setup')) {
	add_action( 'sweet_dessert_action_before_init_theme', 'sweet_dessert_sc_columns_theme_setup' );
	function sweet_dessert_sc_columns_theme_setup() {
		add_action('sweet_dessert_action_shortcodes_list', 		'sweet_dessert_sc_columns_reg_shortcodes');
		if (function_exists('sweet_dessert_exists_visual_composer') && sweet_dessert_exists_visual_composer())
			add_action('sweet_dessert_action_shortcodes_list_vc','sweet_dessert_sc_columns_reg_shortcodes_vc');
	}
}
...
if (!function_exists('sweet_dessert_sc_columns')) {	
	function sweet_dessert_sc_columns($atts, $content=null){	
		...
		sweet_dessert_storage_set('sc_columns_data', array(
			'counter' => 1,
			'equal_selector' => '',
            'after_span2' => false,
            'after_span3' => false,
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="columns_wrap sc_columns'
					. ' columns_' . (sweet_dessert_param_is_on($fluid) ? 'fluid' : 'nofluid') 
					. (!empty($margins) && sweet_dessert_param_is_off($margins) ? ' no_margins' : '') 
					. ' sc_columns_count_' . esc_attr($count)
					. (!empty($class) ? ' '.esc_attr($class) : '') 
				. '"'
				. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
				. (!sweet_dessert_param_is_off($equalheight) ? ' data-equal-height="'.esc_attr(sweet_dessert_storage_get_array('sc_columns_data', 'equal_selector')).'"' : '')
				. (!sweet_dessert_param_is_off($animation) ? ' data-animation="'.esc_attr(sweet_dessert_get_animation_classes($animation)).'"' : '')
				. '>'
					. trim($content)
				. '</div>';
		return apply_filters('sweet_dessert_shortcode_output', $output, 'trx_columns', $atts, $content);
	}
	add_shortcode('trx_columns', 'sweet_dessert_sc_columns');
}


if (!function_exists('sweet_dessert_sc_column_item')) {	
	function sweet_dessert_sc_column_item($atts, $content=null) {
		if (sweet_dessert_in_shortcode_blogger()) return '';
		extract(sweet_dessert_html_decode(shortcode_atts( array(
			// Individual params
			"span" => "1",
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') . ' class="column-'.($span > 1 ? esc_attr($span) : 1).'_'.esc_attr(sweet_dessert_storage_get_array('sc_columns_data', 'count')).' sc_column_item sc_column_item_'.esc_attr(sweet_dessert_storage_get_array('sc_columns_data', 'counter')) 
					. (!empty($class) ? ' '.esc_attr($class) : '')
					. (sweet_dessert_storage_get_array('sc_columns_data', 'counter') % 2 == 1 ? ' odd' : ' even') 
					. (sweet_dessert_storage_get_array('sc_columns_data', 'counter') == 1 ? ' first' : '') 
					. ($span > 1 ? ' span_'.esc_attr($span) : '') 
					. (sweet_dessert_storage_get_array('sc_columns_data', 'after_span2') ? ' after_span_2' : '') 
					. (sweet_dessert_storage_get_array('sc_columns_data', 'after_span3') ? ' after_span_3' : '') 
					. (sweet_dessert_storage_get_array('sc_columns_data', 'after_span4') ? ' after_span_4' : '') 
					. '"'
					. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
					. (!sweet_dessert_param_is_off($animation) ? ' data-animation="'.esc_attr(sweet_dessert_get_animation_classes($animation)).'"' : '')
					. '>'
					...
					. '</div>';
		sweet_dessert_storage_inc_array('sc_columns_data', 'counter', $span);
		sweet_dessert_storage_set_array('sc_columns_data', 'after_span2', $span==2);
		sweet_dessert_storage_set_array('sc_columns_data', 'after_span3', $span==3);
		sweet_dessert_storage_set_array('sc_columns_data', 'after_span4', $span==4);
		sweet_dessert_storage_set_array('sc_columns_data', 'equal_selector', $bg_color!=='' || $bg_image !== '' ? '.sc_column_item_inner' : '.sc_column_item');
		return apply_filters('sweet_dessert_shortcode_output', $output, 'trx_column_item', $atts, $content);
	}
	add_shortcode('trx_column_item', 'sweet_dessert_sc_column_item');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_columns_reg_shortcodes' ) ) {
	function sweet_dessert_sc_columns_reg_shortcodes() {
	
		sweet_dessert_sc_map("trx_columns", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_columns_reg_shortcodes_vc' ) ) {
	function sweet_dessert_sc_columns_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Columns extends Sweet_Dessert_VC_ShortCodeColumns {}
		class WPBakeryShortCode_Trx_Column_Item extends Sweet_Dessert_VC_ShortCodeCollection {}
	}
}
?>
...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('sweet_dessert_sc_audio_theme_setup')) {
	add_action( 'sweet_dessert_action_before_init_theme', 'sweet_dessert_sc_audio_theme_setup' );
	function sweet_dessert_sc_audio_theme_setup() {
		add_action('sweet_dessert_action_shortcodes_list', 		'sweet_dessert_sc_audio_reg_shortcodes');
		if (function_exists('sweet_dessert_exists_visual_composer') && sweet_dessert_exists_visual_composer())
			add_action('sweet_dessert_action_shortcodes_list_vc','sweet_dessert_sc_audio_reg_shortcodes_vc');
	}
}
...
if (!function_exists('sweet_dessert_sc_audio')) {
	function sweet_dessert_sc_audio($atts, $content = null) {
		...
		$audio = '<audio'
			...
			. ' src="'.esc_url($src).'"'
			. (sweet_dessert_param_is_on($controls) ? ' controls="controls"' : '')
			. (sweet_dessert_param_is_on($autoplay) && is_single() ? ' autoplay="autoplay"' : '')
			. ' width="'.esc_attr($width).'" height="'.esc_attr($height).'"'
			. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
			. ($data)
			. '></audio>';
		if ( sweet_dessert_get_custom_option('substitute_audio')=='no') {
			if (sweet_dessert_param_is_on($frame)) {
				$audio = sweet_dessert_get_audio_frame($audio, $image, $s);
			}
		} else {
			if ((isset($_GET['vc_editable']) && $_GET['vc_editable']=='true') && (isset($_POST['action']) && $_POST['action']=='vc_load_shortcode')) {
				$audio = sweet_dessert_substitute_audio($audio, false);
			}
		}
		if (sweet_dessert_get_theme_option('use_mediaelement')=='yes')
			wp_enqueue_script('wp-mediaelement');
		return apply_filters('sweet_dessert_shortcode_output', $audio, 'trx_audio', $atts, $content);
	}
	add_shortcode("trx_audio", "sweet_dessert_sc_audio");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_audio_reg_shortcodes' ) ) {
	function sweet_dessert_sc_audio_reg_shortcodes() {
	
		sweet_dessert_sc_map("trx_audio", array(
			...
			"params" => array(
				...
				"controls" => array(
					...
					"value" => "show",
					"type" => "switch",
					"options" => sweet_dessert_get_sc_param('show_hide')
				),
				"autoplay" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_audio_reg_shortcodes_vc' ) ) {
	function sweet_dessert_sc_audio_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Audio extends Sweet_Dessert_VC_ShortCodeSingle {}
	}
}
?>
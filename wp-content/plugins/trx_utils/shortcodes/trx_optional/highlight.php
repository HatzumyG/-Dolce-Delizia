...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('sweet_dessert_sc_highlight_theme_setup')) {
	add_action( 'sweet_dessert_action_before_init_theme', 'sweet_dessert_sc_highlight_theme_setup' );
	function sweet_dessert_sc_highlight_theme_setup() {
		add_action('sweet_dessert_action_shortcodes_list', 		'sweet_dessert_sc_highlight_reg_shortcodes');
		if (function_exists('sweet_dessert_exists_visual_composer') && sweet_dessert_exists_visual_composer())
			add_action('sweet_dessert_action_shortcodes_list_vc','sweet_dessert_sc_highlight_reg_shortcodes_vc');
	}
}
...
if (!function_exists('sweet_dessert_sc_highlight')) {	
	function sweet_dessert_sc_highlight($atts, $content=null){	
		...
		$css .= ($color != '' ? 'color:' . esc_attr($color) . ';' : '')
			...
			.($font_size != '' ? 'font-size:' . esc_attr(sweet_dessert_prepare_css_value($font_size)) . '; line-height: 1em;' : '');
		$output = '<span' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				...
				. '</span>';
		return apply_filters('sweet_dessert_shortcode_output', $output, 'trx_highlight', $atts, $content);
	}
	add_shortcode('trx_highlight', 'sweet_dessert_sc_highlight');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_highlight_reg_shortcodes' ) ) {
	function sweet_dessert_sc_highlight_reg_shortcodes() {
	
		sweet_dessert_sc_map("trx_highlight", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_highlight_reg_shortcodes_vc' ) ) {
	function sweet_dessert_sc_highlight_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Highlight extends Sweet_Dessert_VC_ShortCodeSingle {}
	}
}
?>
...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('sweet_dessert_sc_icon_theme_setup')) {
	add_action( 'sweet_dessert_action_before_init_theme', 'sweet_dessert_sc_icon_theme_setup' );
	function sweet_dessert_sc_icon_theme_setup() {
		add_action('sweet_dessert_action_shortcodes_list', 		'sweet_dessert_sc_icon_reg_shortcodes');
		if (function_exists('sweet_dessert_exists_visual_composer') && sweet_dessert_exists_visual_composer())
			add_action('sweet_dessert_action_shortcodes_list_vc','sweet_dessert_sc_icon_reg_shortcodes_vc');
	}
}
...
if (!function_exists('sweet_dessert_sc_icon')) {	
	function sweet_dessert_sc_icon($atts, $content=null){	
		...
		$css2 = ($font_weight != '' && !sweet_dessert_is_inherit_option($font_weight) ? 'font-weight:'. esc_attr($font_weight).';' : '')
			. ($font_size != '' ? 'font-size:' . esc_attr(sweet_dessert_prepare_css_value($font_size)) . '; line-height: ' . (!$bg_shape || sweet_dessert_param_is_inherit($bg_shape) ? '1' : '1.2') . 'em;' : '')
		...
		$output = $icon!='' 
			? ($link ? '<a href="'.esc_url($link).'"' : '<span') . ($id ? ' id="'.esc_attr($id).'"' : '')
				. ' class="sc_icon '.esc_attr($icon)
					. ($bg_shape && !sweet_dessert_param_is_inherit($bg_shape) ? ' sc_icon_shape_'.esc_attr($bg_shape) : '')
				...
				.($link ? '</a>' : '</span>')
			: '';
		return apply_filters('sweet_dessert_shortcode_output', $output, 'trx_icon', $atts, $content);
	}
	add_shortcode('trx_icon', 'sweet_dessert_sc_icon');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_icon_reg_shortcodes' ) ) {
	function sweet_dessert_sc_icon_reg_shortcodes() {
	
		sweet_dessert_sc_map("trx_icon", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_icon_reg_shortcodes_vc' ) ) {
	function sweet_dessert_sc_icon_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Icon extends Sweet_Dessert_VC_ShortCodeSingle {}
	}
}
?>
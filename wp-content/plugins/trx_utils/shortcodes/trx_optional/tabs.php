...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('sweet_dessert_sc_tabs_theme_setup')) {
	add_action( 'sweet_dessert_action_before_init_theme', 'sweet_dessert_sc_tabs_theme_setup' );
	function sweet_dessert_sc_tabs_theme_setup() {
		add_action('sweet_dessert_action_shortcodes_list', 		'sweet_dessert_sc_tabs_reg_shortcodes');
		if (function_exists('sweet_dessert_exists_visual_composer') && sweet_dessert_exists_visual_composer())
			add_action('sweet_dessert_action_shortcodes_list_vc','sweet_dessert_sc_tabs_reg_shortcodes_vc');
	}
}
...
if (!function_exists('sweet_dessert_sc_tabs')) {	
	function sweet_dessert_sc_tabs($atts, $content = null) {
		...
		sweet_dessert_storage_set('sc_tab_data', array(
			'counter'=> 0,
            'scroll' => $scroll,
		...
		$sc_tab_titles = sweet_dessert_storage_get_array('sc_tab_data', 'titles');
		...
		$tabs_output .= $titles_output
			...
			. ($content)
			.'</div>';
		return apply_filters('sweet_dessert_shortcode_output', $tabs_output, 'trx_tabs', $atts, $content);
	}
	add_shortcode("trx_tabs", "sweet_dessert_sc_tabs");
}


if (!function_exists('sweet_dessert_sc_tab')) {	
	function sweet_dessert_sc_tab($atts, $content = null) {
		if (sweet_dessert_in_shortcode_blogger()) return '';
		extract(sweet_dessert_html_decode(shortcode_atts(array(
			// Individual params
		...
		sweet_dessert_storage_inc_array('sc_tab_data', 'counter');
		$counter = sweet_dessert_storage_get_array('sc_tab_data', 'counter');
		if (empty($id))
			$id = !empty($tab_id) ? $tab_id : sweet_dessert_storage_get_array('sc_tab_data', 'id').'_'.intval($counter);
		$sc_tab_titles = sweet_dessert_storage_get_array('sc_tab_data', 'titles');
		...
		sweet_dessert_storage_set_array('sc_tab_data', 'titles', $sc_tab_titles);
		$output = '<div id="'.esc_attr($id).'"'
					.' class="sc_tabs_content' 
						...
						. ($css!='' ? ' style="'.esc_attr($css).'"' : '') 
						. '>' 
				. (sweet_dessert_param_is_on(sweet_dessert_storage_get_array('sc_tab_data', 'scroll')) 
					? '<div id="'.esc_attr($id).'_scroll" class="sc_scroll sc_scroll_vertical" style="height:'.(sweet_dessert_storage_get_array('sc_tab_data', 'height') != '' ? sweet_dessert_storage_get_array('sc_tab_data', 'height') : '200px').';"><div class="sc_scroll_wrapper swiper-wrapper"><div class="sc_scroll_slide swiper-slide">' 
					: '')
				...
				. (sweet_dessert_param_is_on(sweet_dessert_storage_get_array('sc_tab_data', 'scroll')) 
					...
					: '')
			. '</div>';
		return apply_filters('sweet_dessert_shortcode_output', $output, 'trx_tab', $atts, $content);
	}
	add_shortcode("trx_tab", "sweet_dessert_sc_tab");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_tabs_reg_shortcodes' ) ) {
	function sweet_dessert_sc_tabs_reg_shortcodes() {
	
		sweet_dessert_sc_map("trx_tabs", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'sweet_dessert_sc_tabs_reg_shortcodes_vc' ) ) {
	function sweet_dessert_sc_tabs_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Tabs extends Sweet_Dessert_VC_ShortCodeTabs {}
		class WPBakeryShortCode_Trx_Tab extends Sweet_Dessert_VC_ShortCodeTab {}
	}
}
?>
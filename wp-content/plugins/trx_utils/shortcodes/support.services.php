<?php
/**
 * Sweet Dessert Framework: Services support
 *
...
// Theme init
if (!function_exists('sweet_dessert_services_theme_setup')) {
	add_action( 'sweet_dessert_action_before_init_theme', 'sweet_dessert_services_theme_setup',1 );
	function sweet_dessert_services_theme_setup() {
		
		// Detect current page type, taxonomy and title (for custom post_types use priority < 10 to fire it handles early, than for standard post types)
		add_filter('sweet_dessert_filter_get_blog_type',			'sweet_dessert_services_get_blog_type', 9, 2);
		add_filter('sweet_dessert_filter_get_blog_title',		'sweet_dessert_services_get_blog_title', 9, 2);
		add_filter('sweet_dessert_filter_get_current_taxonomy',	'sweet_dessert_services_get_current_taxonomy', 9, 2);
		add_filter('sweet_dessert_filter_is_taxonomy',			'sweet_dessert_services_is_taxonomy', 9, 2);
		add_filter('sweet_dessert_filter_get_stream_page_title',	'sweet_dessert_services_get_stream_page_title', 9, 2);
		add_filter('sweet_dessert_filter_get_stream_page_link',	'sweet_dessert_services_get_stream_page_link', 9, 2);
		add_filter('sweet_dessert_filter_get_stream_page_id',	'sweet_dessert_services_get_stream_page_id', 9, 2);
		add_filter('sweet_dessert_filter_query_add_filters',		'sweet_dessert_services_query_add_filters', 9, 2);
		add_filter('sweet_dessert_filter_detect_inheritance_key','sweet_dessert_services_detect_inheritance_key', 9, 1);

		// Extra column for services lists
		if (sweet_dessert_get_theme_option('show_overriden_posts')=='yes') {
			add_filter('manage_edit-services_columns',			'sweet_dessert_post_add_options_column', 9);
			add_filter('manage_services_posts_custom_column',	'sweet_dessert_post_fill_options_column', 9, 2);
		}

		// Register shortcodes [trx_services] and [trx_services_item]
		add_action('sweet_dessert_action_shortcodes_list',		'sweet_dessert_services_reg_shortcodes');
		if (function_exists('sweet_dessert_exists_visual_composer') && sweet_dessert_exists_visual_composer())
			add_action('sweet_dessert_action_shortcodes_list_vc','sweet_dessert_services_reg_shortcodes_vc');
		
		// Add supported data types
		sweet_dessert_theme_support_pt('services');
		sweet_dessert_theme_support_tx('services_group');
	}
}

if ( !function_exists( 'sweet_dessert_services_settings_theme_setup2' ) ) {
	add_action( 'sweet_dessert_action_before_init_theme', 'sweet_dessert_services_settings_theme_setup2', 3 );
	function sweet_dessert_services_settings_theme_setup2() {
		// Add post type 'services' and taxonomy 'services_group' into theme inheritance list
		sweet_dessert_add_theme_inheritance( array('services' => array(
...
if ( !function_exists( 'sweet_dessert_is_services_page' ) ) {
	function sweet_dessert_is_services_page() {
		$is = in_array(sweet_dessert_storage_get('page_template'), array('blog-services', 'single-service'));
		if (!$is) {
			if (!sweet_dessert_storage_empty('pre_query'))
				$is = sweet_dessert_storage_call_obj_method('pre_query', 'get', 'post_type')=='services' 
						|| sweet_dessert_storage_call_obj_method('pre_query', 'is_tax', 'services_group') 
						|| (sweet_dessert_storage_call_obj_method('pre_query', 'is_page') 
								&& ($id=sweet_dessert_get_template_page_id('blog-services')) > 0 
				...
				$is = get_query_var('post_type')=='services' 
						...
						|| (is_page() && ($id=sweet_dessert_get_template_page_id('blog-services')) > 0 && $id==get_the_ID());
		}
		return $is;
	}
}
...
if ( !function_exists( 'sweet_dessert_services_detect_inheritance_key' ) ) {
	function sweet_dessert_services_detect_inheritance_key($key) {
		if (!empty($key)) return $key;
		return sweet_dessert_is_services_page() ? 'services' : '';
	}
}

// Filter to detect current page slug
if ( !function_exists( 'sweet_dessert_services_get_blog_type' ) ) {
	function sweet_dessert_services_get_blog_type($page, $query=null) {
...
if ( !function_exists( 'sweet_dessert_services_get_blog_title' ) ) {
	function sweet_dessert_services_get_blog_title($title, $page) {
		if (!empty($title)) return $title;
		if ( sweet_dessert_strpos($page, 'services')!==false ) {
...
if ( !function_exists( 'sweet_dessert_services_get_stream_page_title' ) ) {
	function sweet_dessert_services_get_stream_page_title($title, $page) {
		if (!empty($title)) return $title;
		if (sweet_dessert_strpos($page, 'services')!==false) {
			if (($page_id = sweet_dessert_services_get_stream_page_id(0, $page=='services' ? 'blog-services' : $page)) > 0)
...
if ( !function_exists( 'sweet_dessert_services_get_stream_page_id' ) ) {
	function sweet_dessert_services_get_stream_page_id($id, $page) {
		if (!empty($id)) return $id;
		if (sweet_dessert_strpos($page, 'services')!==false) $id = sweet_dessert_get_template_page_id('blog-services');
		return $id;
	}
}

// Filter to detect stream page URL
if ( !function_exists( 'sweet_dessert_services_get_stream_page_link' ) ) {
	function sweet_dessert_services_get_stream_page_link($url, $page) {
		if (!empty($url)) return $url;
		if (sweet_dessert_strpos($page, 'services')!==false) {
			$id = sweet_dessert_get_template_page_id('blog-services');
...
if ( !function_exists( 'sweet_dessert_services_get_current_taxonomy' ) ) {
	function sweet_dessert_services_get_current_taxonomy($tax, $page) {
...
if ( !function_exists( 'sweet_dessert_services_is_taxonomy' ) ) {
	function sweet_dessert_services_is_taxonomy($tax, $query=null) {
		if (!empty($tax))
			return $tax;
		else 
...
if ( !function_exists( 'sweet_dessert_services_query_add_filters' ) ) {
	function sweet_dessert_services_query_add_filters($args, $filter) {
		if ($filter == 'services') {
...
if ( !function_exists( 'sweet_dessert_sc_services' ) ) {
	function sweet_dessert_sc_services($atts, $content=null){	
		...
		if (sweet_dessert_param_is_off($slider) && $columns > 1 && $style == 'services-5' && !empty($image)) $columns = 2;
		if (!empty($image)) {
			if ($image > 0) {
		...
		if (!empty($height) && sweet_dessert_param_is_on($autoheight)) $autoheight = "no";
		if (empty($interval)) $interval = mt_rand(5000, 10000);
		
		$class .= ($class ? ' ' : '') . sweet_dessert_get_css_position_as_classes($top, $right, $bottom, $left);

		$ws = sweet_dessert_get_css_dimensions_from_values($width);
		$hs = sweet_dessert_get_css_dimensions_from_values('', $height);
		$css .= ($hs) . ($ws);

		$columns = max(1, min(12, (int) $columns));
		$count = max(1, (int) $count);
		if (sweet_dessert_param_is_off($custom) && $count < $columns) $columns = $count;

		if (sweet_dessert_param_is_on($slider)) sweet_dessert_enqueue_slider('swiper');

		sweet_dessert_storage_set('sc_services_data', array(
			'id' => $id,
            'style' => $style,
            'type' => $type,
            'columns' => $columns,
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'_wrap"' : '') 
					...
					. '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
						. ' class="sc_services'
							...
							. ' ' . esc_attr(sweet_dessert_get_template_property($style, 'container_classes'))
							. (!empty($class) ? ' '.esc_attr($class) : '')
							. ($align!='' && $align!='none' ? ' align'.esc_attr($align) : '')
							. '"'
						. ($css!='' ? ' style="'.esc_attr($css).'"' : '') 
						. (!sweet_dessert_param_is_off($equalheight) ? ' data-equal-height=".sc_services_item"' : '')
						. (!sweet_dessert_param_is_off($animation) ? ' data-animation="'.esc_attr(sweet_dessert_get_animation_classes($animation)).'"' : '')
					. '>'
					. (!empty($subtitle) ? '<h6 class="sc_services_subtitle sc_item_subtitle">' . trim(sweet_dessert_strmacros($subtitle)) . '</h6>' : '')
					. (!empty($title) ? '<h2 class="sc_services_title sc_item_title' . (empty($description) ? ' sc_item_title_without_descr' : ' sc_item_title_with_descr') . '">' . trim(sweet_dessert_strmacros($title)) . '</h2>' : '')
					. (!empty($description) ? '<div class="sc_services_descr sc_item_descr">' . trim(sweet_dessert_strmacros($description)) . '</div>' : '')
					. (sweet_dessert_param_is_on($slider) 
						? ('<div class="sc_slider_swiper swiper-slider-container'
										. ' ' . esc_attr(sweet_dessert_get_slider_controls_classes($controls))
										. (sweet_dessert_param_is_on($autoheight) ? ' sc_slider_height_auto' : '')
										. ($hs ? ' sc_slider_height_fixed' : '')
										. '"'
									. (!empty($width) && sweet_dessert_strpos($width, '%')===false ? ' data-old-width="' . esc_attr($width) . '"' : '')
									. (!empty($height) && sweet_dessert_strpos($height, '%')===false ? ' data-old-height="' . esc_attr($height) . '"' : '')
		...
		if (sweet_dessert_param_is_on($custom) && $content) {
			...
			$args = sweet_dessert_query_add_sort_order($args, $orderby, $order);
			$args = sweet_dessert_query_add_posts_and_cats($args, $ids, 'services', $cat, 'services_group');
			...
			while ( $query->have_posts() ) { 
				...
				$args = array(
					...
					"descr" => sweet_dessert_get_custom_option('post_excerpt_maxlength'.($columns > 1 ? '_masonry' : '')),
					"orderby" => $orderby,
					'content' => false,
					'terms_list' => false,
					'readmore' => $readmore,
				...
				$output .= sweet_dessert_show_post_layout($args);
			}
			wp_reset_postdata();
		}
	
		if (sweet_dessert_param_is_on($slider)) {
			$output .= '</div>'
		...
		$output .=  (!empty($link) ? '<div class="sc_services_button sc_item_button">'.sweet_dessert_do_shortcode('[trx_button link="'.esc_url($link).'" icon="icon-right"]'.esc_html($link_caption).'[/trx_button]').'</div>' : '')
					. '</div><!-- /.sc_services -->'
				. '</div><!-- /.sc_services_wrap -->';
	
		// Add template specific scripts and styles
		do_action('sweet_dessert_action_blog_scripts', $style);
	
		return apply_filters('sweet_dessert_shortcode_output', $output, 'trx_services', $atts, $content);
	}
	add_shortcode('trx_services', 'sweet_dessert_sc_services');
}


if ( !function_exists( 'sweet_dessert_sc_services_item' ) ) {
	function sweet_dessert_sc_services_item($atts, $content=null) {
		if (sweet_dessert_in_shortcode_blogger()) return '';
		extract(sweet_dessert_html_decode(shortcode_atts( array(
			// Individual params
			"icon" => "",
		...
		sweet_dessert_storage_inc_array('sc_services_data', 'counter');

		$id = $id ? $id : (sweet_dessert_storage_get_array('sc_services_data', 'id') ? sweet_dessert_storage_get_array('sc_services_data', 'id') . '_' . sweet_dessert_storage_get_array('sc_services_data', 'counter') : '');

		$descr = trim(chop(do_shortcode($content)));
		$readmore = $readmore=='(none)' ? sweet_dessert_storage_get_array('sc_services_data', 'readmore') : $readmore;

		$type = sweet_dessert_storage_get_array('sc_services_data', 'type');
		if (!empty($icon)) {
			$type = 'icons';
		} else if (!empty($image)) {
			$type = 'images';
			...
			$thumb_sizes = sweet_dessert_get_thumb_sizes(array('layout' => sweet_dessert_storage_get_array('sc_services_data', 'style')));
			$image = sweet_dessert_get_resized_image_tag($image, $thumb_sizes['w'], $thumb_sizes['h']);
		}
		...
		$args = array(
			'layout' => sweet_dessert_storage_get_array('sc_services_data', 'style'),
			'number' => sweet_dessert_storage_get_array('sc_services_data', 'counter'),
			'columns_count' => sweet_dessert_storage_get_array('sc_services_data', 'columns'),
			'slider' => sweet_dessert_storage_get_array('sc_services_data', 'slider'),
			'show' => false,
			...
			'tag_css' => $css,
			'tag_css_wh' => sweet_dessert_storage_get_array('sc_services_data', 'css_wh')
		);
		$output = sweet_dessert_show_post_layout($args, $post_data);
		return apply_filters('sweet_dessert_shortcode_output', $output, 'trx_services_item', $atts, $content);
	}
	add_shortcode('trx_services_item', 'sweet_dessert_sc_services_item');
}
// ---------------------------------- [/trx_services] ---------------------------------------
...
if (!function_exists('sweet_dessert_services_reg_shortcodes')) {
	function sweet_dessert_services_reg_shortcodes() {
		if (sweet_dessert_storage_isset('shortcodes')) {

			$services_groups = sweet_dessert_get_list_terms(false, 'services_group');
			$services_styles = sweet_dessert_get_list_templates('services');
			$controls 		 = sweet_dessert_get_list_slider_controls();

			sweet_dessert_sc_map_after('trx_section', array(
				...
				"trx_services" => array(
					...
					"params" => array(
						...
						"cat" => array(
							...
							"style" => "list",
							"multiple" => true,
							"options" => sweet_dessert_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), $services_groups)
						),
						"count" => array(
						...
						"orderby" => array(
							...
							"value" => "date",
							"type" => "select",
							"options" => sweet_dessert_get_sc_param('sorting')
						),
						"order" => array(
...
if (!function_exists('sweet_dessert_services_reg_shortcodes_vc')) {
	function sweet_dessert_services_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"params" => array(
					...
					array(
						...
						"class" => "",
						"std" => "no",
						"value" => array_flip(sweet_dessert_get_sc_param('yes_no')),
						"type" => "dropdown"
					),
					array(
						"param_name" => "controls",
					...
					array(
						...
						"class" => "",
						"value" => array_flip(sweet_dessert_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), $services_groups)),
						"type" => "dropdown"
					),
					array(
						"param_name" => "columns",
					...
					array(
						...
						"std" => "date",
						"class" => "",
						"value" => array_flip(sweet_dessert_get_sc_param('sorting')),
						"type" => "dropdown"
					),
					array(
						"param_name" => "order",
						...
						"std" => "desc",
						"class" => "",
						"value" => array_flip(sweet_dessert_get_sc_param('ordering')),
						"type" => "dropdown"
					),
					array(
						"param_name" => "ids",
		...
		class WPBakeryShortCode_Trx_Services extends Sweet_Dessert_VC_ShortCodeColumns {}
		class WPBakeryShortCode_Trx_Services_Item extends Sweet_Dessert_VC_ShortCodeCollection {}

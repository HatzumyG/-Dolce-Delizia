...
// Theme init
if (!function_exists('sweet_dessert_widget_socials_theme_setup')) {
	add_action( 'sweet_dessert_action_before_init_theme', 'sweet_dessert_widget_socials_theme_setup', 1 );
	function sweet_dessert_widget_socials_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('sweet_dessert_exists_visual_composer') && sweet_dessert_exists_visual_composer())
			add_action('sweet_dessert_action_shortcodes_list_vc','sweet_dessert_widget_socials_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('sweet_dessert_widget_socials_load')) {
	add_action( 'widgets_init', 'sweet_dessert_widget_socials_load' );
	function sweet_dessert_widget_socials_load() {
		register_widget( 'sweet_dessert_widget_socials' );
	}
}

// Widget Class
class sweet_dessert_widget_socials extends WP_Widget {

	function __construct() {
		$widget_ops = array( 'classname' => 'widget_socials', 'description' => esc_html__('Show site logo and social links', 'trx_utils') );
		parent::__construct( 'sweet_dessert_widget_socials', esc_html__('Sweet Dessert - Show logo and social links', 'trx_utils'), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$text = isset($instance['text']) ? sweet_dessert_do_shortcode($instance['text']) : '';
		...
		sweet_dessert_show_layout($before_widget);

		// Display the widget title if one was input (before and after defined by themes)
		if ($title) sweet_dessert_show_layout($title, $before_title, $after_title);
            ...
            <?php
				if ($show_logo) {
					...
					if ($logo_image || $logo_text)
						sweet_dessert_show_logo($logo_image, false, false, false, $logo_text, $logo_slogan);
				}
				...
				if ($show_icons) {
					sweet_dessert_show_layout(sweet_dessert_sc_socials(array('size' => "tiny", 'shape' => 'round')));
				}
			?>
		</div>
		...
		sweet_dessert_show_layout($after_widget);
	}
	...
	function form( $instance ) {
		...
		<p>
            ...
            <?php
			sweet_dessert_show_layout(sweet_dessert_show_custom_field($this->get_field_id( 'logo_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'logo_image' )), null));
			if ($logo_image) {
			?>
...
*/
if ( !function_exists( 'sweet_dessert_sc_widget_socials' ) ) {
	function sweet_dessert_sc_widget_socials($atts, $content=null){	
		$atts = sweet_dessert_html_decode(shortcode_atts(array(
			// Individual params
			"title" => "",
			"text" => "",
			"logo_image" => "",
		...
		extract($atts);
		$type = 'sweet_dessert_widget_socials';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_socials' 
								. (sweet_dessert_exists_visual_composer() ? ' vc_widget_socials wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, sweet_dessert_prepare_widgets_args(sweet_dessert_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_socials', 'widget_socials') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('sweet_dessert_shortcode_output', $output, 'trx_widget_socials', $atts, $content);
	}
	add_shortcode("trx_widget_socials", "sweet_dessert_sc_widget_socials");
}


// Add [trx_widget_socials] in the VC shortcodes list
if (!function_exists('sweet_dessert_widget_socials_reg_shortcodes_vc')) {
	function sweet_dessert_widget_socials_reg_shortcodes_vc() {
